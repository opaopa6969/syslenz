...
- generated_at: 2026-04-02T20:30:14+09:00

## en
### Raw (All Lines)
- nonempty lines: 88922
- unique lines: 14322
- duplicated lines: 74600
- duplication ratio: 83.89%

### Framework-Excluded (Signal Lines)
- nonempty lines: 57619
- unique lines: 12977
- duplicated lines: 44642
- duplication ratio: 77.48%
...
- [167] - If uncertain, follow SEE ALSO links before changing production vmstat-related memory knobs.
- [167] - If traffic had stayed constant, would vmstat reclaim and scan transitions still move this field the same way?
- [167] - If this field had remained flat, which non-vmstat signal could still explain the symptom?
- [167] - If only one layer could be instrumented, which vmstat-adjacent layer would preserve most explanatory power?
- [167] - If mitigation was delayed by 10 minutes, which vmstat-linked user metric would have crossed first?
- [167] - Domain: virtual memory and reclaim behavior
- [167] - Cross-check: scheduler delay and storage writeback side effects.
- [167] - Confusing cross-layer vmstat correlation with causation.

## ja
### Raw (All Lines)
- nonempty lines: 88923
- unique lines: 11524
- duplicated lines: 77399
- duplication ratio: 87.04%

### Framework-Excluded (Signal Lines)
- nonempty lines: 57620
- unique lines: 10178
- duplicated lines: 47442
- duplication ratio: 82.34%
...
- [167] - 分岐1: 症状は改善したが vmstat トレンドは不変。 -> vmstat 因果レイヤー仮説を再検証。
- [167] - 併読: scheduler 遅延と storage writeback 副作用。
- [167] - 主な契機: メモリ圧力やワークロード位相変化。
- [167] - 中程度: 相関はあるが vmstat reclaim/scan 境界が曖昧
- [167] - リリース、カーネル、アロケータ挙動変更後は vmstat 関連閾値を再検証する。
- [167] - トラフィック一定でも、vmstat の reclaim/scan 遷移はこのfieldを同方向に動かすか？
- [167] - カーネル経路: reclaim、compaction、page fault、swap 制御。
- [167] - まね調整を避け、vmstat+pressure 文脈付きの before/after 証拠を要求する。
