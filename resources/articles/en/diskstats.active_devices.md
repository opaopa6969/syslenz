...
## NAME
`diskstats.active_devices` - metric signal from `diskstats` (active_devices)
...
For `active_devices`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `active_devices`.
...
### Decision Record
- Primary claim: diskstats.active_devices indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: active_devices was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (Storage)
Field under practice: active_devices
