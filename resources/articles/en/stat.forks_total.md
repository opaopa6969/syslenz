...
## NAME
`stat.forks_total` - metric signal from `stat` (forks_total)
...
This signal (stat.forks_total) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat forks_total as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: stat.forks_total indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: forks_total was treated as evidence in a chain, not a singleton verdict.
