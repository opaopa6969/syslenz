...
## NAME
`processes.process_count` - metric signal from `processes` (process_count)
...
3. Pair one sibling metric in `processes` plus one pressure/queue metric from another source.
...
This signal (processes.process_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat process_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: processes.process_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: process_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: process_count
- Source anchor: processes
