...
## NAME
`loadavg.last_pid` - metric signal from `loadavg` (last_pid)
...
This signal (loadavg.last_pid) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat last_pid as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: loadavg.last_pid indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: last_pid was treated as evidence in a chain, not a singleton verdict.
