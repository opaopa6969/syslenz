...
## NAME
`ss.tcp_orphaned` - metric signal from `ss` (tcp_orphaned)
...
This signal (ss.tcp_orphaned) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat tcp_orphaned as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: ss.tcp_orphaned indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: tcp_orphaned was treated as evidence in a chain, not a singleton verdict.
