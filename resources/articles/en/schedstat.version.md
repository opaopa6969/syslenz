...
## NAME
`schedstat.version` - metric signal from `schedstat` (version)
...
This signal (schedstat.version) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat version as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: schedstat.version indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: version was treated as evidence in a chain, not a singleton verdict.
