...
## NAME
`interrupts.cpu_count` - metric signal from `interrupts` (cpu_count)
...
This signal (interrupts.cpu_count) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: interrupts.cpu_count indicated a meaningful state transition.
