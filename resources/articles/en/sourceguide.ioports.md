# Source Guide: ioports

## NAME
`sourceguide.ioports` - source-level operational reading guide
...
`ioports` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.ioports) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat ioports as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for ioports.
...
### Decision Record
- Primary claim: sourceguide.ioports indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: ioports was treated as evidence in a chain, not a singleton verdict.
