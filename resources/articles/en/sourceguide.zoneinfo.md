# Source Guide: zoneinfo

## NAME
`sourceguide.zoneinfo` - source-level operational reading guide
...
`zoneinfo` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.zoneinfo) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat zoneinfo as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for zoneinfo.
...
### Decision Record
- Primary claim: sourceguide.zoneinfo indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: zoneinfo was treated as evidence in a chain, not a singleton verdict.
