...
## NAME
`vmstat.nr_unstable` - metric signal from `vmstat` (nr_unstable)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `nr_unstable` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `nr_unstable` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.nr_unstable`
- Source: `vmstat` (`nr_unstable`)
- Field: `nr_unstable`
...
This field is strongest for separating vmstat reclaim signal around `nr_unstable` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `nr_unstable`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `nr_unstable`.
...
This field is a manifestation of **virtual memory state transitions (`nr_unstable`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`nr_unstable`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `nr_unstable`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `nr_unstable`
- T-5m: first vmstat reclaim or scan anomaly candidate around `nr_unstable`
- T0: user symptom confirmed with vmstat-side context around `nr_unstable`
- T+3m: first hypothesis written with vmstat reclaim assumption for `nr_unstable`
- T+6m: cross-source validation including scheduler or writeback for `nr_unstable`
- T+10m: mitigation applied with rollback guard in the vmstat path for `nr_unstable`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `nr_unstable`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `nr_unstable`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `nr_unstable`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `nr_unstable`
...
### Decision Record
- Primary claim: vmstat.nr_unstable indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: nr_unstable was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `nr_unstable`.
...
- Field anchor: nr_unstable
- Source anchor: vmstat (`nr_unstable`)
...
### Debrief Questions
- Which vmstat-side step around `nr_unstable` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `nr_unstable` versus side effect?
- What vmstat instrumentation change around `nr_unstable` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: nr_unstable

## Failure Archetype Matrix
- Archetype A (`nr_unstable`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`nr_unstable`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`nr_unstable`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: nr_unstable
