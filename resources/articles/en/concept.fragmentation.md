...
## NAME
`concept.fragmentation` - foundational systems concept for diagnosis
...
## Core Thesis
`Memory Fragmentation` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.fragmentation) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat fragmentation as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around fragmentation.
...
### Decision Record
- Primary claim: concept.fragmentation indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: fragmentation was treated as evidence in a chain, not a singleton verdict.
