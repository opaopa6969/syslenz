...
## NAME
`vmstat.pgdeactivate` - metric signal from `vmstat` (pgdeactivate)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pgdeactivate` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pgdeactivate` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.pgdeactivate`
- Source: `vmstat` (`pgdeactivate`)
- Field: `pgdeactivate`
...
This field is strongest for separating vmstat reclaim signal around `pgdeactivate` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `pgdeactivate`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pgdeactivate`.
...
This field is a manifestation of **virtual memory state transitions (`pgdeactivate`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pgdeactivate`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pgdeactivate`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pgdeactivate`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pgdeactivate`
- T0: user symptom confirmed with vmstat-side context around `pgdeactivate`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pgdeactivate`
- T+6m: cross-source validation including scheduler or writeback for `pgdeactivate`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pgdeactivate`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pgdeactivate`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pgdeactivate`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pgdeactivate`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pgdeactivate`
...
### Decision Record
- Primary claim: vmstat.pgdeactivate indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pgdeactivate was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pgdeactivate`.
...
- Field anchor: pgdeactivate
- Source anchor: vmstat (`pgdeactivate`)
...
### Debrief Questions
- Which vmstat-side step around `pgdeactivate` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pgdeactivate` versus side effect?
- What vmstat instrumentation change around `pgdeactivate` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pgdeactivate

## Failure Archetype Matrix
- Archetype A (`pgdeactivate`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pgdeactivate`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pgdeactivate`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pgdeactivate
