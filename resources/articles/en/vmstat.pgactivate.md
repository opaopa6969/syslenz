...
## NAME
`vmstat.pgactivate` - metric signal from `vmstat` (pgactivate)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pgactivate` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pgactivate` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.pgactivate`
- Source: `vmstat` (`pgactivate`)
- Field: `pgactivate`
...
This field is strongest for separating vmstat reclaim signal around `pgactivate` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `pgactivate`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pgactivate`.
...
This field is a manifestation of **virtual memory state transitions (`pgactivate`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pgactivate`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pgactivate`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pgactivate`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pgactivate`
- T0: user symptom confirmed with vmstat-side context around `pgactivate`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pgactivate`
- T+6m: cross-source validation including scheduler or writeback for `pgactivate`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pgactivate`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pgactivate`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pgactivate`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pgactivate`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pgactivate`
...
### Decision Record
- Primary claim: vmstat.pgactivate indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pgactivate was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pgactivate`.
...
- Field anchor: pgactivate
- Source anchor: vmstat (`pgactivate`)
...
### Debrief Questions
- Which vmstat-side step around `pgactivate` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pgactivate` versus side effect?
- What vmstat instrumentation change around `pgactivate` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pgactivate

## Failure Archetype Matrix
- Archetype A (`pgactivate`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pgactivate`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pgactivate`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pgactivate
