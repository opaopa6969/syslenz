...
## NAME
`meminfo.KernelStack` - metric signal from `meminfo` (KernelStack)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `KernelStack` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `KernelStack` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `KernelStack` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
Transport counters indicated retry amplification before request-level failures became obvious.

For `KernelStack`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `KernelStack`.
...
This field is a manifestation of **memory accounting surfaces (KernelStack)**.

- Kernel path (KernelStack): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (KernelStack): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (KernelStack): vmstat reclaim counters and pressure metrics.
...
### Decision Record
- Primary claim: meminfo.KernelStack indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: KernelStack was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: KernelStack
