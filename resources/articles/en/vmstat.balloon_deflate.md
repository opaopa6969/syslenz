...
## NAME
`vmstat.balloon_deflate` - metric signal from `vmstat` (balloon_deflate)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `balloon_deflate` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `balloon_deflate` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.balloon_deflate`
- Source: `vmstat` (`balloon_deflate`)
...
This field is strongest for separating vmstat reclaim signal around `balloon_deflate` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `balloon_deflate`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `balloon_deflate`.
...
This field is a manifestation of **virtual memory state transitions (`balloon_deflate`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`balloon_deflate`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `balloon_deflate`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `balloon_deflate`
- T-5m: first vmstat reclaim or scan anomaly candidate around `balloon_deflate`
- T0: user symptom confirmed with vmstat-side context around `balloon_deflate`
- T+3m: first hypothesis written with vmstat reclaim assumption for `balloon_deflate`
- T+6m: cross-source validation including scheduler or writeback for `balloon_deflate`
- T+10m: mitigation applied with rollback guard in the vmstat path for `balloon_deflate`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `balloon_deflate`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `balloon_deflate`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `balloon_deflate`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `balloon_deflate`
...
### Decision Record
- Primary claim: vmstat.balloon_deflate indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: balloon_deflate was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `balloon_deflate`.
...
- Field anchor: balloon_deflate
- Source anchor: vmstat (`balloon_deflate`)
...
### Debrief Questions
- Which vmstat-side step around `balloon_deflate` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `balloon_deflate` versus side effect?
- What vmstat instrumentation change around `balloon_deflate` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: balloon_deflate

## Failure Archetype Matrix
- Archetype A (`balloon_deflate`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`balloon_deflate`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`balloon_deflate`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: balloon_deflate
