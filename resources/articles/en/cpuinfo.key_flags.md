...
## NAME
`cpuinfo.key_flags` - metric signal from `cpuinfo` (key_flags)
...
This signal (cpuinfo.key_flags) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat key_flags as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: cpuinfo.key_flags indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: key_flags was treated as evidence in a chain, not a singleton verdict.
