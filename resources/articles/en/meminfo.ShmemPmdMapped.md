...
## NAME
`meminfo.ShmemPmdMapped` - metric signal from `meminfo` (ShmemPmdMapped)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `ShmemPmdMapped` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `ShmemPmdMapped` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `ShmemPmdMapped` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `ShmemPmdMapped`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `ShmemPmdMapped`.
...
This field is a manifestation of **memory accounting surfaces (ShmemPmdMapped)**.

- Kernel path (ShmemPmdMapped): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (ShmemPmdMapped): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (ShmemPmdMapped): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case C (`ShmemPmdMapped`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 2 (MemInfo)
Case A (`ShmemPmdMapped`): First anomaly came from this field trend, not absolute value.

### Incident Slice 3 (MemInfo)
Case B (`ShmemPmdMapped`): Cross-source correlation reversed the initial diagnosis.
...
### Decision Record
- Primary claim: meminfo.ShmemPmdMapped indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: ShmemPmdMapped was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: ShmemPmdMapped
