# Source Guide: diskstats

## NAME
`sourceguide.diskstats` - source-level operational reading guide
...
`diskstats` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.diskstats) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat diskstats as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for diskstats.
...
### Episode 1
Write wait expanded before throughput fell. Queue depth plus wait distribution explained why average throughput looked acceptable while p99 degraded.

### Episode 2
The effective action was smoothing write bursts rather than tuning every filesystem knob.
...
### Decision Record
- Primary claim: sourceguide.diskstats indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: diskstats was treated as evidence in a chain, not a singleton verdict.
