...
## NAME
`vmstat.pgfree` - metric signal from `vmstat` (pgfree)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pgfree` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pgfree` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating vmstat reclaim signal around `pgfree` from side-effect noise when scan and reclaim trends diverge.
...
For `pgfree`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pgfree`.
...
This field is a manifestation of **virtual memory state transitions (`pgfree`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pgfree`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pgfree`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pgfree`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pgfree`
- T0: user symptom confirmed with vmstat-side context around `pgfree`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pgfree`
- T+6m: cross-source validation including scheduler or writeback for `pgfree`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pgfree`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pgfree`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pgfree`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pgfree`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pgfree`
...
### Decision Record
- Primary claim: vmstat.pgfree indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pgfree was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pgfree`.
...
### Debrief Questions
- Which vmstat-side step around `pgfree` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pgfree` versus side effect?
- What vmstat instrumentation change around `pgfree` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pgfree

## Failure Archetype Matrix
- Archetype A (`pgfree`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pgfree`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pgfree`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pgfree
