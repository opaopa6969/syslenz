...
## NAME
`sourceguide.iomem` - source-level operational reading guide
...
`iomem` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.iomem) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat iomem as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for iomem.
...
### Decision Record
- Primary claim: sourceguide.iomem indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: iomem was treated as evidence in a chain, not a singleton verdict.
