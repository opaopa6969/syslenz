...
## NAME
`misc.device_count` - metric signal from `misc` (device_count)
...
3. Pair one sibling metric in `misc` plus one pressure/queue metric from another source.
...
This signal (misc.device_count) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: misc.device_count indicated a meaningful state transition.
