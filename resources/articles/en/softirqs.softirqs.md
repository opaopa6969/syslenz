...
## NAME
`softirqs.softirqs` - metric signal from `softirqs` (softirqs)
...
This signal (softirqs.softirqs) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat softirqs as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: softirqs.softirqs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: softirqs was treated as evidence in a chain, not a singleton verdict.
