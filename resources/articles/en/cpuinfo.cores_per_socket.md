...
## NAME
`cpuinfo.cores_per_socket` - metric signal from `cpuinfo` (cores_per_socket)
...
3. Pair one sibling metric in `cpuinfo` plus one pressure/queue metric from another source.
...
This signal (cpuinfo.cores_per_socket) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cores_per_socket as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: cpuinfo.cores_per_socket indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cores_per_socket was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: cores_per_socket
- Source anchor: cpuinfo
...
- Field in focus: cores_per_socket
