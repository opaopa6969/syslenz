...
## NAME
`cmdline.cmdline` - metric signal from `cmdline` (cmdline)
...
This signal (cmdline.cmdline) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: cmdline.cmdline indicated a meaningful state transition.
