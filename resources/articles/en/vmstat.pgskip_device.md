...
## NAME
`vmstat.pgskip_device` - metric signal from `vmstat` (pgskip_device)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pgskip_device` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pgskip_device` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.pgskip_device`
- Source: `vmstat` (`pgskip_device`)
- Field: `pgskip_device`
...
This field is strongest for separating vmstat reclaim signal around `pgskip_device` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `pgskip_device`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pgskip_device`.
...
This field is a manifestation of **virtual memory state transitions (`pgskip_device`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pgskip_device`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pgskip_device`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pgskip_device`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pgskip_device`
- T0: user symptom confirmed with vmstat-side context around `pgskip_device`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pgskip_device`
- T+6m: cross-source validation including scheduler or writeback for `pgskip_device`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pgskip_device`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pgskip_device`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pgskip_device`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pgskip_device`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pgskip_device`
...
### Decision Record
- Primary claim: vmstat.pgskip_device indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pgskip_device was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pgskip_device`.
...
- Field anchor: pgskip_device
- Source anchor: vmstat (`pgskip_device`)
...
### Debrief Questions
- Which vmstat-side step around `pgskip_device` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pgskip_device` versus side effect?
- What vmstat instrumentation change around `pgskip_device` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pgskip_device

## Failure Archetype Matrix
- Archetype A (`pgskip_device`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pgskip_device`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pgskip_device`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pgskip_device
