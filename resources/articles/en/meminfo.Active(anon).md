...
## NAME
`meminfo.Active(anon)` - metric signal from `meminfo` (Active(anon))

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `Active(anon)` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `Active(anon)` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `Active(anon)` from side-effect noise when free/reclaimable trends diverge.
...
For `Active(anon)`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Active(anon)`.
...
This field is a manifestation of **memory accounting surfaces (Active(anon))**.

- Kernel path (Active(anon)): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (Active(anon)): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (Active(anon)): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case C (`Active(anon)`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 2 (MemInfo)
Case A (`Active(anon)`): First anomaly came from this field trend, not absolute value.

### Incident Slice 3 (MemInfo)
Case B (`Active(anon)`): Cross-source correlation reversed the initial diagnosis.
...
### Decision Record
- Primary claim: meminfo.Active(anon) indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Active(anon) was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: Active(anon)
