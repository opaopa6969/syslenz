...
## NAME
`meminfo.SUnreclaim` - metric signal from `meminfo` (SUnreclaim)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `SUnreclaim` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `SUnreclaim` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `SUnreclaim` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
During a latency spike, reclaim-related counters moved before user-visible error rate changed.

For `SUnreclaim`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `SUnreclaim`.
...
This field is a manifestation of **memory accounting surfaces (SUnreclaim)**.

- Kernel path (SUnreclaim): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (SUnreclaim): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (SUnreclaim): vmstat reclaim counters and pressure metrics.
...
### Decision Record
- Primary claim: meminfo.SUnreclaim indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: SUnreclaim was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: SUnreclaim
