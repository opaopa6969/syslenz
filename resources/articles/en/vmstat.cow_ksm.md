...
## NAME
`vmstat.cow_ksm` - metric signal from `vmstat` (cow_ksm)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `cow_ksm` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `cow_ksm` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating vmstat reclaim signal around `cow_ksm` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `cow_ksm`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `cow_ksm`.
...
This field is a manifestation of **virtual memory state transitions (`cow_ksm`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`cow_ksm`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `cow_ksm`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `cow_ksm`
- T-5m: first vmstat reclaim or scan anomaly candidate around `cow_ksm`
- T0: user symptom confirmed with vmstat-side context around `cow_ksm`
- T+3m: first hypothesis written with vmstat reclaim assumption for `cow_ksm`
- T+6m: cross-source validation including scheduler or writeback for `cow_ksm`
- T+10m: mitigation applied with rollback guard in the vmstat path for `cow_ksm`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `cow_ksm`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `cow_ksm`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `cow_ksm`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `cow_ksm`
...
### Decision Record
- Primary claim: vmstat.cow_ksm indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cow_ksm was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `cow_ksm`.
...
- Field anchor: cow_ksm
- Source anchor: vmstat (`cow_ksm`)
...
### Debrief Questions
- Which vmstat-side step around `cow_ksm` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `cow_ksm` versus side effect?
- What vmstat instrumentation change around `cow_ksm` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: cow_ksm

## Failure Archetype Matrix
- Archetype A (`cow_ksm`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`cow_ksm`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`cow_ksm`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: cow_ksm
