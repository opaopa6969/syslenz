...
## NAME
`vmstat.nr_vmscan_write` - metric signal from `vmstat` (nr_vmscan_write)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `nr_vmscan_write` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `nr_vmscan_write` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.nr_vmscan_write`
- Source: `vmstat` (`nr_vmscan_write`)
...
This field is strongest for separating vmstat reclaim signal around `nr_vmscan_write` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
During a latency spike, reclaim-related counters moved before user-visible error rate changed.

For `nr_vmscan_write`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `nr_vmscan_write`.
...
This field is a manifestation of **virtual memory state transitions (`nr_vmscan_write`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`nr_vmscan_write`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `nr_vmscan_write`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `nr_vmscan_write`
- T-5m: first vmstat reclaim or scan anomaly candidate around `nr_vmscan_write`
- T0: user symptom confirmed with vmstat-side context around `nr_vmscan_write`
- T+3m: first hypothesis written with vmstat reclaim assumption for `nr_vmscan_write`
- T+6m: cross-source validation including scheduler or writeback for `nr_vmscan_write`
- T+10m: mitigation applied with rollback guard in the vmstat path for `nr_vmscan_write`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `nr_vmscan_write`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `nr_vmscan_write`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `nr_vmscan_write`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `nr_vmscan_write`
...
### Decision Record
- Primary claim: vmstat.nr_vmscan_write indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: nr_vmscan_write was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `nr_vmscan_write`.
...
- Field anchor: nr_vmscan_write
- Source anchor: vmstat (`nr_vmscan_write`)
...
### Debrief Questions
- Which vmstat-side step around `nr_vmscan_write` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `nr_vmscan_write` versus side effect?
- What vmstat instrumentation change around `nr_vmscan_write` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: nr_vmscan_write

## Failure Archetype Matrix
- Archetype A (`nr_vmscan_write`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`nr_vmscan_write`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`nr_vmscan_write`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: nr_vmscan_write
