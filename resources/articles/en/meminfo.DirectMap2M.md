...
## NAME
`meminfo.DirectMap2M` - metric signal from `meminfo` (DirectMap2M)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `DirectMap2M` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `DirectMap2M` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `DirectMap2M` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `DirectMap2M`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `DirectMap2M`.
...
This field is a manifestation of **memory accounting surfaces (DirectMap2M)**.

- Kernel path (DirectMap2M): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (DirectMap2M): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (DirectMap2M): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case C (`DirectMap2M`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 2 (MemInfo)
Case A (`DirectMap2M`): First anomaly came from this field trend, not absolute value.

### Incident Slice 3 (MemInfo)
Case B (`DirectMap2M`): Cross-source correlation reversed the initial diagnosis.
...
### Decision Record
- Primary claim: meminfo.DirectMap2M indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: DirectMap2M was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: DirectMap2M
