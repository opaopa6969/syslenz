...
## NAME
`concept.cpu-runqueue` - foundational systems concept for diagnosis
...
## Core Thesis
`CPU Runqueue` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.cpu-runqueue) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cpu-runqueue as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around cpu-runqueue.
...
### Decision Record
- Primary claim: concept.cpu-runqueue indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cpu-runqueue was treated as evidence in a chain, not a singleton verdict.
