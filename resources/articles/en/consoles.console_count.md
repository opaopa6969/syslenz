...
## NAME
`consoles.console_count` - metric signal from `consoles` (console_count)
...
3. Pair one sibling metric in `consoles` plus one pressure/queue metric from another source.
...
This signal (consoles.console_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat console_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: consoles.console_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: console_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: console_count
- Source anchor: consoles
