...
## NAME
`meminfo.Mapped` - metric signal from `meminfo` (Mapped)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `Mapped` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `Mapped` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `Mapped` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `Mapped`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Mapped`.
...
This field is a manifestation of **memory accounting surfaces (Mapped)**.

- Kernel path (Mapped): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (Mapped): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (Mapped): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case C (`Mapped`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 2 (MemInfo)
Case A (`Mapped`): First anomaly came from this field trend, not absolute value.

### Incident Slice 3 (MemInfo)
Case B (`Mapped`): Cross-source correlation reversed the initial diagnosis.
...
### Decision Record
- Primary claim: meminfo.Mapped indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Mapped was treated as evidence in a chain, not a singleton verdict.
