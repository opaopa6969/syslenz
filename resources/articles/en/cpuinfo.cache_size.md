...
## NAME
`cpuinfo.cache_size` - metric signal from `cpuinfo` (cache_size)
...
This signal (cpuinfo.cache_size) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cache_size as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: cpuinfo.cache_size indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cache_size was treated as evidence in a chain, not a singleton verdict.
