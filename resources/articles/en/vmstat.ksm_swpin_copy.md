...
## NAME
`vmstat.ksm_swpin_copy` - metric signal from `vmstat` (ksm_swpin_copy)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `ksm_swpin_copy` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `ksm_swpin_copy` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.ksm_swpin_copy`
- Source: `vmstat` (`ksm_swpin_copy`)
- Field: `ksm_swpin_copy`
...
This field is strongest for separating vmstat reclaim signal around `ksm_swpin_copy` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
Swap-related movement stayed low at first, then accelerated when cache headroom collapsed.

For `ksm_swpin_copy`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `ksm_swpin_copy`.
...
This field is a manifestation of **virtual memory state transitions (`ksm_swpin_copy`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`ksm_swpin_copy`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `ksm_swpin_copy`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `ksm_swpin_copy`
- T-5m: first vmstat reclaim or scan anomaly candidate around `ksm_swpin_copy`
- T0: user symptom confirmed with vmstat-side context around `ksm_swpin_copy`
- T+3m: first hypothesis written with vmstat reclaim assumption for `ksm_swpin_copy`
- T+6m: cross-source validation including scheduler or writeback for `ksm_swpin_copy`
- T+10m: mitigation applied with rollback guard in the vmstat path for `ksm_swpin_copy`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `ksm_swpin_copy`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `ksm_swpin_copy`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `ksm_swpin_copy`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `ksm_swpin_copy`
...
### Decision Record
- Primary claim: vmstat.ksm_swpin_copy indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: ksm_swpin_copy was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `ksm_swpin_copy`.
...
- Field anchor: ksm_swpin_copy
- Source anchor: vmstat (`ksm_swpin_copy`)
...
### Debrief Questions
- Which vmstat-side step around `ksm_swpin_copy` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `ksm_swpin_copy` versus side effect?
- What vmstat instrumentation change around `ksm_swpin_copy` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: ksm_swpin_copy

## Failure Archetype Matrix
- Archetype A (`ksm_swpin_copy`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`ksm_swpin_copy`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`ksm_swpin_copy`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: ksm_swpin_copy
