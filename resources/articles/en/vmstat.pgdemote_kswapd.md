...
## NAME
`vmstat.pgdemote_kswapd` - metric signal from `vmstat` (pgdemote_kswapd)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pgdemote_kswapd` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pgdemote_kswapd` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.pgdemote_kswapd`
- Source: `vmstat` (`pgdemote_kswapd`)
...
This field is strongest for separating vmstat reclaim signal around `pgdemote_kswapd` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
Swap-related movement stayed low at first, then accelerated when cache headroom collapsed.

For `pgdemote_kswapd`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pgdemote_kswapd`.
...
This field is a manifestation of **virtual memory state transitions (`pgdemote_kswapd`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pgdemote_kswapd`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pgdemote_kswapd`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pgdemote_kswapd`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pgdemote_kswapd`
- T0: user symptom confirmed with vmstat-side context around `pgdemote_kswapd`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pgdemote_kswapd`
- T+6m: cross-source validation including scheduler or writeback for `pgdemote_kswapd`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pgdemote_kswapd`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pgdemote_kswapd`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pgdemote_kswapd`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pgdemote_kswapd`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pgdemote_kswapd`
...
### Decision Record
- Primary claim: vmstat.pgdemote_kswapd indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pgdemote_kswapd was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pgdemote_kswapd`.
...
- Field anchor: pgdemote_kswapd
- Source anchor: vmstat (`pgdemote_kswapd`)
...
### Debrief Questions
- Which vmstat-side step around `pgdemote_kswapd` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pgdemote_kswapd` versus side effect?
- What vmstat instrumentation change around `pgdemote_kswapd` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pgdemote_kswapd

## Failure Archetype Matrix
- Archetype A (`pgdemote_kswapd`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pgdemote_kswapd`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pgdemote_kswapd`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pgdemote_kswapd
