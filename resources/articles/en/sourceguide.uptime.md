...
## NAME
`sourceguide.uptime` - source-level operational reading guide
...
`uptime` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.uptime) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat  18:22:33 up 2 days,  8:34,  2 users,  load average: 0.47, 0.58, 0.58 as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for uptime.
...
### Decision Record
- Primary claim: sourceguide.uptime indicated a meaningful state transition.
