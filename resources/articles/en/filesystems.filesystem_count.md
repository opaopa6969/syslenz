...
## NAME
`filesystems.filesystem_count` - metric signal from `filesystems` (filesystem_count)
...
## Metric Snapshot
- ID: `filesystems.filesystem_count`
- Source: `filesystems`
- Field: `filesystem_count`
...
3. Pair one sibling metric in `filesystems` plus one pressure/queue metric from another source.
...
This signal (filesystems.filesystem_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat filesystem_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: filesystems.filesystem_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: filesystem_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: filesystem_count
- Source anchor: filesystems
...
- Field in focus: filesystem_count
