...
## NAME
`swaps.swap_areas` - metric signal from `swaps` (swap_areas)
...
This signal (swaps.swap_areas) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat swap_areas as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: swaps.swap_areas indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: swap_areas was treated as evidence in a chain, not a singleton verdict.
