...
## NAME
`devices.devices` - metric signal from `devices` (devices)
...
This signal (devices.devices) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: devices.devices indicated a meaningful state transition.
