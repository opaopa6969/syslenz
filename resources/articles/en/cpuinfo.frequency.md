...
## NAME
`cpuinfo.frequency` - metric signal from `cpuinfo` (frequency)
...
This signal (cpuinfo.frequency) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat frequency as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: cpuinfo.frequency indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: frequency was treated as evidence in a chain, not a singleton verdict.
