...
## NAME
`vmstat.oom_kill` - metric signal from `vmstat` (oom_kill)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `oom_kill` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `oom_kill` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating vmstat reclaim signal around `oom_kill` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `oom_kill`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `oom_kill`.
...
This field is a manifestation of **virtual memory state transitions (`oom_kill`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`oom_kill`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `oom_kill`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `oom_kill`
- T-5m: first vmstat reclaim or scan anomaly candidate around `oom_kill`
- T0: user symptom confirmed with vmstat-side context around `oom_kill`
- T+3m: first hypothesis written with vmstat reclaim assumption for `oom_kill`
- T+6m: cross-source validation including scheduler or writeback for `oom_kill`
- T+10m: mitigation applied with rollback guard in the vmstat path for `oom_kill`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `oom_kill`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `oom_kill`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `oom_kill`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `oom_kill`
...
### Decision Record
- Primary claim: vmstat.oom_kill indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: oom_kill was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `oom_kill`.
...
- Field anchor: oom_kill
- Source anchor: vmstat (`oom_kill`)
...
### Debrief Questions
- Which vmstat-side step around `oom_kill` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `oom_kill` versus side effect?
- What vmstat instrumentation change around `oom_kill` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: oom_kill

## Failure Archetype Matrix
- Archetype A (`oom_kill`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`oom_kill`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`oom_kill`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: oom_kill
