...
## NAME
`df.total_disk` - metric signal from `df` (total_disk)
...
This signal (df.total_disk) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat total_disk as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: df.total_disk indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: total_disk was treated as evidence in a chain, not a singleton verdict.
