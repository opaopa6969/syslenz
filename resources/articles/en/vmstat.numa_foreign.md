...
## NAME
`vmstat.numa_foreign` - metric signal from `vmstat` (numa_foreign)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `numa_foreign` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `numa_foreign` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.numa_foreign`
- Source: `vmstat` (`numa_foreign`)
- Field: `numa_foreign`
...
This field is strongest for separating vmstat reclaim signal around `numa_foreign` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `numa_foreign`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `numa_foreign`.
...
This field is a manifestation of **virtual memory state transitions (`numa_foreign`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`numa_foreign`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `numa_foreign`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `numa_foreign`
- T-5m: first vmstat reclaim or scan anomaly candidate around `numa_foreign`
- T0: user symptom confirmed with vmstat-side context around `numa_foreign`
- T+3m: first hypothesis written with vmstat reclaim assumption for `numa_foreign`
- T+6m: cross-source validation including scheduler or writeback for `numa_foreign`
- T+10m: mitigation applied with rollback guard in the vmstat path for `numa_foreign`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `numa_foreign`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `numa_foreign`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `numa_foreign`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `numa_foreign`
...
### Decision Record
- Primary claim: vmstat.numa_foreign indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: numa_foreign was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `numa_foreign`.
...
- Field anchor: numa_foreign
- Source anchor: vmstat (`numa_foreign`)
...
### Debrief Questions
- Which vmstat-side step around `numa_foreign` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `numa_foreign` versus side effect?
- What vmstat instrumentation change around `numa_foreign` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: numa_foreign

## Failure Archetype Matrix
- Archetype A (`numa_foreign`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`numa_foreign`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`numa_foreign`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: numa_foreign
