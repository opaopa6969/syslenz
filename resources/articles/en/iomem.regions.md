...
## NAME
`iomem.regions` - metric signal from `iomem` (regions)
...
This signal (iomem.regions) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: iomem.regions indicated a meaningful state transition.
