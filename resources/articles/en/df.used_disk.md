...
## NAME
`df.used_disk` - metric signal from `df` (used_disk)
...
This signal (df.used_disk) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat used_disk as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: df.used_disk indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: used_disk was treated as evidence in a chain, not a singleton verdict.
