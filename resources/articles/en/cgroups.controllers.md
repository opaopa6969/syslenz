...
## NAME
`cgroups.controllers` - metric signal from `cgroups` (controllers)
...
This signal (cgroups.controllers) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat controllers as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: cgroups.controllers indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: controllers was treated as evidence in a chain, not a singleton verdict.
