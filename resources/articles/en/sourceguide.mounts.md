...
## NAME
`sourceguide.mounts` - source-level operational reading guide
...
`mounts` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.mounts) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat mounts as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for mounts.
...
### Decision Record
- Primary claim: sourceguide.mounts indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: mounts was treated as evidence in a chain, not a singleton verdict.
