...
## NAME
`meminfo.Percpu` - metric signal from `meminfo` (Percpu)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `Percpu` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `Percpu` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `Percpu` from side-effect noise when free/reclaimable trends diverge.
...
For `Percpu`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Percpu`.
...
This field is a manifestation of **memory accounting surfaces (Percpu)**.

- Kernel path (Percpu): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (Percpu): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (Percpu): vmstat reclaim counters and pressure metrics.
...
### Decision Record
- Primary claim: meminfo.Percpu indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Percpu was treated as evidence in a chain, not a singleton verdict.
