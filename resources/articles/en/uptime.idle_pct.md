...
## NAME
`uptime.idle_pct` - metric signal from `uptime` (idle_pct)
...
This signal (uptime.idle_pct) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat idle_pct as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: uptime.idle_pct indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: idle_pct was treated as evidence in a chain, not a singleton verdict.
