...
## NAME
`vmstat.numa_hit` - metric signal from `vmstat` (numa_hit)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `numa_hit` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `numa_hit` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating vmstat reclaim signal around `numa_hit` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `numa_hit`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `numa_hit`.
...
This field is a manifestation of **virtual memory state transitions (`numa_hit`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`numa_hit`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `numa_hit`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `numa_hit`
- T-5m: first vmstat reclaim or scan anomaly candidate around `numa_hit`
- T0: user symptom confirmed with vmstat-side context around `numa_hit`
- T+3m: first hypothesis written with vmstat reclaim assumption for `numa_hit`
- T+6m: cross-source validation including scheduler or writeback for `numa_hit`
- T+10m: mitigation applied with rollback guard in the vmstat path for `numa_hit`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `numa_hit`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `numa_hit`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `numa_hit`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `numa_hit`
...
### Decision Record
- Primary claim: vmstat.numa_hit indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: numa_hit was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `numa_hit`.
...
- Field anchor: numa_hit
- Source anchor: vmstat (`numa_hit`)
...
### Debrief Questions
- Which vmstat-side step around `numa_hit` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `numa_hit` versus side effect?
- What vmstat instrumentation change around `numa_hit` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: numa_hit

## Failure Archetype Matrix
- Archetype A (`numa_hit`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`numa_hit`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`numa_hit`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: numa_hit
