...
## NAME
`meminfo.MemFree` - metric signal from `meminfo` (MemFree)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `MemFree` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `MemFree` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `MemFree` from side-effect noise when free/reclaimable trends diverge.
...
For `MemFree`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `MemFree`.
...
This field is a manifestation of **memory accounting surfaces (MemFree)**.

- Kernel path (MemFree): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (MemFree): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (MemFree): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case B (`MemFree`): Cross-source correlation reversed the initial diagnosis.

### Incident Slice 2 (MemInfo)
Case C (`MemFree`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 3 (MemInfo)
Case A (`MemFree`): First anomaly came from this field trend, not absolute value.
...
### Decision Record
- Primary claim: meminfo.MemFree indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: MemFree was treated as evidence in a chain, not a singleton verdict.
