# Source Guide: consoles

## NAME
`sourceguide.consoles` - source-level operational reading guide
...
`consoles` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.consoles) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat consoles as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for consoles.
...
### Decision Record
- Primary claim: sourceguide.consoles indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: consoles was treated as evidence in a chain, not a singleton verdict.
