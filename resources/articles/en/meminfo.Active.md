...
## NAME
`meminfo.Active` - metric signal from `meminfo` (Active)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `Active` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `Active` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `Active` from side-effect noise when free/reclaimable trends diverge.
...
For `Active`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Active`.
...
This field is a manifestation of **memory accounting surfaces (Active)**.

- Kernel path (Active): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (Active): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (Active): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case B (`Active`): Cross-source correlation reversed the initial diagnosis.

### Incident Slice 2 (MemInfo)
Case C (`Active`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 3 (MemInfo)
Case A (`Active`): First anomaly came from this field trend, not absolute value.
...
### Decision Record
- Primary claim: meminfo.Active indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Active was treated as evidence in a chain, not a singleton verdict.
