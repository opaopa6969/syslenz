...
## NAME
`mounts.mounts` - metric signal from `mounts` (mounts)
...
3. Pair one sibling metric in `mounts` plus one pressure/queue metric from another source.
...
This signal (mounts.mounts) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: mounts.mounts indicated a meaningful state transition.
