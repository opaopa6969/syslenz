...
## NAME
`vmstat.workingset_nodereclaim` - metric signal from `vmstat` (workingset_nodereclaim)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `workingset_nodereclaim` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `workingset_nodereclaim` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.workingset_nodereclaim`
- Source: `vmstat` (`workingset_nodereclaim`)
...
This field is strongest for separating vmstat reclaim signal around `workingset_nodereclaim` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
During a latency spike, reclaim-related counters moved before user-visible error rate changed.

For `workingset_nodereclaim`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `workingset_nodereclaim`.
...
This field is a manifestation of **virtual memory state transitions (`workingset_nodereclaim`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`workingset_nodereclaim`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `workingset_nodereclaim`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `workingset_nodereclaim`
- T-5m: first vmstat reclaim or scan anomaly candidate around `workingset_nodereclaim`
- T0: user symptom confirmed with vmstat-side context around `workingset_nodereclaim`
- T+3m: first hypothesis written with vmstat reclaim assumption for `workingset_nodereclaim`
- T+6m: cross-source validation including scheduler or writeback for `workingset_nodereclaim`
- T+10m: mitigation applied with rollback guard in the vmstat path for `workingset_nodereclaim`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `workingset_nodereclaim`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `workingset_nodereclaim`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `workingset_nodereclaim`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `workingset_nodereclaim`
...
### Decision Record
- Primary claim: vmstat.workingset_nodereclaim indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: workingset_nodereclaim was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `workingset_nodereclaim`.
...
- Field anchor: workingset_nodereclaim
- Source anchor: vmstat (`workingset_nodereclaim`)
...
### Debrief Questions
- Which vmstat-side step around `workingset_nodereclaim` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `workingset_nodereclaim` versus side effect?
- What vmstat instrumentation change around `workingset_nodereclaim` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: workingset_nodereclaim

## Failure Archetype Matrix
- Archetype A (`workingset_nodereclaim`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`workingset_nodereclaim`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`workingset_nodereclaim`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: workingset_nodereclaim
