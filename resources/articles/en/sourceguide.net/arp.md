# Source Guide: net/arp

## NAME
`sourceguide.net/arp` - source-level operational reading guide
...
`net/arp` captures protocol and path-level evidence that often appears before application error rates rise.
...
This signal (sourceguide.net/arp) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat net/arp as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for net/arp.
...
### Decision Record
- Primary claim: sourceguide.net/arp indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: net/arp was treated as evidence in a chain, not a singleton verdict.
