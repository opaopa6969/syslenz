# Source Guide: net/tcp

## NAME
`sourceguide.net/tcp` - source-level operational reading guide
...
`net/tcp` captures protocol and path-level evidence that often appears before application error rates rise.
...
This signal (sourceguide.net/tcp) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat net/tcp as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for net/tcp.
...
### Decision Record
- Primary claim: sourceguide.net/tcp indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: net/tcp was treated as evidence in a chain, not a singleton verdict.
