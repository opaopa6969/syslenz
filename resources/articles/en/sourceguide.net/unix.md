# Source Guide: net/unix

## NAME
`sourceguide.net/unix` - source-level operational reading guide
...
`net/unix` captures protocol and path-level evidence that often appears before application error rates rise.
...
This signal (sourceguide.net/unix) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat net/unix as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for net/unix.
...
### Decision Record
- Primary claim: sourceguide.net/unix indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: net/unix was treated as evidence in a chain, not a singleton verdict.
