# Source Guide: net/sockstat

## NAME
`sourceguide.net/sockstat` - source-level operational reading guide
...
`net/sockstat` captures protocol and path-level evidence that often appears before application error rates rise.
...
This signal (sourceguide.net/sockstat) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat net/sockstat as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for net/sockstat.
...
### Decision Record
- Primary claim: sourceguide.net/sockstat indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: net/sockstat was treated as evidence in a chain, not a singleton verdict.
