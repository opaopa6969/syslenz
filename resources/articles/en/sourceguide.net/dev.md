# Source Guide: net/dev

## NAME
`sourceguide.net/dev` - source-level operational reading guide
...
`net/dev` captures protocol and path-level evidence that often appears before application error rates rise.
...
This signal (sourceguide.net/dev) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat net/dev as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for net/dev.
...
### Decision Record
- Primary claim: sourceguide.net/dev indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: net/dev was treated as evidence in a chain, not a singleton verdict.
