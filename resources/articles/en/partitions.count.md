...
## NAME
`partitions.count` - metric signal from `partitions` (count)
...
This signal (partitions.count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: partitions.count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: count was treated as evidence in a chain, not a singleton verdict.
