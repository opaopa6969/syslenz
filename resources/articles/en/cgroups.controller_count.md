...
## NAME
`cgroups.controller_count` - metric signal from `cgroups` (controller_count)
...
## Metric Snapshot
- ID: `cgroups.controller_count`
- Source: `cgroups`
- Field: `controller_count`
...
3. Pair one sibling metric in `cgroups` plus one pressure/queue metric from another source.
...
This signal (cgroups.controller_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat controller_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: cgroups.controller_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: controller_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: controller_count
- Source anchor: cgroups
...
- Field in focus: controller_count
