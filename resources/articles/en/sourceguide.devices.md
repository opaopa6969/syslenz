# Source Guide: devices

## NAME
`sourceguide.devices` - source-level operational reading guide
...
`devices` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.devices) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat devices as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for devices.
...
### Decision Record
- Primary claim: sourceguide.devices indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: devices was treated as evidence in a chain, not a singleton verdict.
