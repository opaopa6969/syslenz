...
## NAME
`meminfo.NFS_Unstable` - metric signal from `meminfo` (NFS_Unstable)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `NFS_Unstable` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `NFS_Unstable` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `NFS_Unstable` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `NFS_Unstable`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `NFS_Unstable`.
...
This field is a manifestation of **memory accounting surfaces (NFS_Unstable)**.

- Kernel path (NFS_Unstable): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (NFS_Unstable): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (NFS_Unstable): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case A (`NFS_Unstable`): First anomaly came from this field trend, not absolute value.

### Incident Slice 2 (MemInfo)
Case B (`NFS_Unstable`): Cross-source correlation reversed the initial diagnosis.

### Incident Slice 3 (MemInfo)
Case C (`NFS_Unstable`): Reversible mitigation provided faster learning than invasive change.
...
### Decision Record
- Primary claim: meminfo.NFS_Unstable indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: NFS_Unstable was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: NFS_Unstable
