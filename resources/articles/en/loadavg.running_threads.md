...
## NAME
`loadavg.running_threads` - metric signal from `loadavg` (running_threads)
...
This signal (loadavg.running_threads) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat running_threads as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: loadavg.running_threads indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: running_threads was treated as evidence in a chain, not a singleton verdict.
...
- Field in focus: running_threads
