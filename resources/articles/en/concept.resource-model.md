...
## NAME
`concept.resource-model` - foundational systems concept for diagnosis
...
## Core Thesis
`Resource Model` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.resource-model) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat resource-model as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around resource-model.
...
### Decision Record
- Primary claim: concept.resource-model indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: resource-model was treated as evidence in a chain, not a singleton verdict.
