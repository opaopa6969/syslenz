...
## NAME
`conntrack.usage_pct` - metric signal from `conntrack` (usage_pct)
...
This signal (conntrack.usage_pct) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat usage_pct as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: conntrack.usage_pct indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: usage_pct was treated as evidence in a chain, not a singleton verdict.
