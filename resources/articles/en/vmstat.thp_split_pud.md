...
## NAME
`vmstat.thp_split_pud` - metric signal from `vmstat` (thp_split_pud)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `thp_split_pud` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `thp_split_pud` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.thp_split_pud`
- Source: `vmstat` (`thp_split_pud`)
- Field: `thp_split_pud`
- Domain: virtual memory and reclaim behavior
- Signal family: hugepage/compaction dynamics

## Operational Meaning (VMStat Lens)
This field is strongest for separating vmstat reclaim signal around `thp_split_pud` from side-effect noise when scan and reclaim trends diverge.
...
For `thp_split_pud`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `thp_split_pud`.
...
This field is a manifestation of **virtual memory state transitions (`thp_split_pud`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`thp_split_pud`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `thp_split_pud`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `thp_split_pud`
- T-5m: first vmstat reclaim or scan anomaly candidate around `thp_split_pud`
- T0: user symptom confirmed with vmstat-side context around `thp_split_pud`
- T+3m: first hypothesis written with vmstat reclaim assumption for `thp_split_pud`
- T+6m: cross-source validation including scheduler or writeback for `thp_split_pud`
- T+10m: mitigation applied with rollback guard in the vmstat path for `thp_split_pud`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `thp_split_pud`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `thp_split_pud`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `thp_split_pud`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `thp_split_pud`
...
### Decision Record
- Primary claim: vmstat.thp_split_pud indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: thp_split_pud was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `thp_split_pud`.
...
- Field anchor: thp_split_pud
- Source anchor: vmstat (`thp_split_pud`)
...
### Debrief Questions
- Which vmstat-side step around `thp_split_pud` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `thp_split_pud` versus side effect?
- What vmstat instrumentation change around `thp_split_pud` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: thp_split_pud

## Failure Archetype Matrix
- Archetype A (`thp_split_pud`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`thp_split_pud`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`thp_split_pud`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: thp_split_pud
