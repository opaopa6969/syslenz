# io_queue_depth_distribution

## NAME
`diskstats.io_queue_depth_distribution` - metric signal from `diskstats` (io_queue_depth_distribution)
...
- Source: `diskstats`
- Field: `io_queue_depth_distribution`
...
For `io_queue_depth_distribution`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `io_queue_depth_distribution`.
...
### Decision Record
- Primary claim: diskstats.io_queue_depth_distribution indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: io_queue_depth_distribution was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: io_queue_depth_distribution
...
### Anchor (Storage)
Field under practice: io_queue_depth_distribution
...
- Field in focus: io_queue_depth_distribution
