...
## NAME
`stat.procs_blocked` - metric signal from `stat` (procs_blocked)
...
This signal (stat.procs_blocked) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat procs_blocked as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: stat.procs_blocked indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: procs_blocked was treated as evidence in a chain, not a singleton verdict.
