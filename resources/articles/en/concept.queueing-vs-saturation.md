...
## NAME
`concept.queueing-vs-saturation` - foundational systems concept for diagnosis
...
## Core Thesis
`Queueing vs Saturation` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.queueing-vs-saturation) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat queueing-vs-saturation as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around queueing-vs-saturation.
...
### Decision Record
- Primary claim: concept.queueing-vs-saturation indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: queueing-vs-saturation was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: queueing-vs-saturation
...
- Field in focus: queueing-vs-saturation
