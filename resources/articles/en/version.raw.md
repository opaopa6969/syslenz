...
## NAME
`version.raw` - metric signal from `version` (raw)
...
This signal (version.raw) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat raw as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: version.raw indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: raw was treated as evidence in a chain, not a singleton verdict.
