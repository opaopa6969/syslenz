# I/O Path: From Syscall to Driver and Device Queue

## NAME
`concept.io-path-syscall-to-device` - foundational systems concept for diagnosis
...
## Big Picture
A `read` or `write` call does not go straight to hardware. It traverses VFS, page cache, block layer, driver queue, then device firmware/hardware.

## Why Incidents Need This Model
Bottlenecks migrate across layers:
- app timeout symptom
- kernel queue growth
- driver/device service-time variance

Without path awareness, teams patch the wrong layer.

## Episode
A storage incident showed stable throughput with unstable latency.
- app logs suggested random timeouts.
- block queue metrics showed bursty wait tails.
- device-level behavior had periodic stalls.

Fix targeted queue depth and request shaping, not only app retry logic.

## Reading Checklist
1. request latency distribution.
2. queue depth / in-flight trends.
3. read/write wait distributions.
4. correlation with reclaim/writeback side effects.

## Mistakes
- assuming throughput means healthy latency.
- blaming driver before checking queueing upstream.
- adding retries that increase queue pressure.

## Systems Narrative (Storage)

This signal (concept.io-path-syscall-to-device) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat io-path-syscall-to-device as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around io-path-syscall-to-device.
...
### Decision Record
- Primary claim: concept.io-path-syscall-to-device indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: io-path-syscall-to-device was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: io-path-syscall-to-device
...
- Field in focus: io-path-syscall-to-device
