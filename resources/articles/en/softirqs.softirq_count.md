...
## NAME
`softirqs.softirq_count` - metric signal from `softirqs` (softirq_count)
...
3. Pair one sibling metric in `softirqs` plus one pressure/queue metric from another source.
...
This field is a manifestation of **deferred kernel work backlog**.

- Kernel path: NET_RX/NET_TX/TIMER and related deferred handlers.
- Typical trigger: packet bursts, timer storms, NAPI pressure.
- Cross-check: interrupts and protocol counters.

## Systems Narrative (Process)

This signal (softirqs.softirq_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat softirq_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: softirqs.softirq_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: softirq_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: softirq_count
- Source anchor: softirqs
