# runqueue_distribution

## NAME
`schedstat.runqueue_distribution` - metric signal from `schedstat` (runqueue_distribution)
...
This signal (schedstat.runqueue_distribution) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat runqueue_distribution as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: schedstat.runqueue_distribution indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: runqueue_distribution was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: runqueue_distribution
...
- Field in focus: runqueue_distribution
