...
## NAME
`meminfo.VmallocUsed` - metric signal from `meminfo` (VmallocUsed)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `VmallocUsed` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `VmallocUsed` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `VmallocUsed` from side-effect noise when free/reclaimable trends diverge.
...
For `VmallocUsed`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `VmallocUsed`.
...
This field is a manifestation of **memory accounting surfaces (VmallocUsed)**.

- Kernel path (VmallocUsed): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (VmallocUsed): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (VmallocUsed): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case B (`VmallocUsed`): Cross-source correlation reversed the initial diagnosis.

### Incident Slice 2 (MemInfo)
Case C (`VmallocUsed`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 3 (MemInfo)
Case A (`VmallocUsed`): First anomaly came from this field trend, not absolute value.
...
### Decision Record
- Primary claim: meminfo.VmallocUsed indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: VmallocUsed was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: VmallocUsed
