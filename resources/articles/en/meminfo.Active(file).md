...
## NAME
`meminfo.Active(file)` - metric signal from `meminfo` (Active(file))

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `Active(file)` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `Active(file)` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `Active(file)` from side-effect noise when free/reclaimable trends diverge.
...
For `Active(file)`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Active(file)`.
...
This field is a manifestation of **memory accounting surfaces (Active(file))**.

- Kernel path (Active(file)): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (Active(file)): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (Active(file)): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case A (`Active(file)`): First anomaly came from this field trend, not absolute value.

### Incident Slice 2 (MemInfo)
Case B (`Active(file)`): Cross-source correlation reversed the initial diagnosis.

### Incident Slice 3 (MemInfo)
Case C (`Active(file)`): Reversible mitigation provided faster learning than invasive change.
...
### Decision Record
- Primary claim: meminfo.Active(file) indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Active(file) was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: Active(file)
