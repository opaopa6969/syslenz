...
## NAME
`meminfo.DirectMap1G` - metric signal from `meminfo` (DirectMap1G)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `DirectMap1G` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `DirectMap1G` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `DirectMap1G` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `DirectMap1G`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `DirectMap1G`.
...
This field is a manifestation of **memory accounting surfaces (DirectMap1G)**.

- Kernel path (DirectMap1G): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (DirectMap1G): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (DirectMap1G): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case C (`DirectMap1G`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 2 (MemInfo)
Case A (`DirectMap1G`): First anomaly came from this field trend, not absolute value.

### Incident Slice 3 (MemInfo)
Case B (`DirectMap1G`): Cross-source correlation reversed the initial diagnosis.
...
### Decision Record
- Primary claim: meminfo.DirectMap1G indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: DirectMap1G was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: DirectMap1G
