...
## NAME
`dns.search_domains` - metric signal from `dns` (search_domains)
...
This signal (dns.search_domains) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat search_domains as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: dns.search_domains indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: search_domains was treated as evidence in a chain, not a singleton verdict.
