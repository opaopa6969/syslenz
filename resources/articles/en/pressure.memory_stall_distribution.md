...
## NAME
`pressure.memory_stall_distribution` - metric signal from `pressure` (memory_stall_distribution)

## WHY NOW
Read this when pressure stalled-time behavior around `memory_stall_distribution` is noisy and you need a fast, defensible decision.
If you cannot tell pressure stall signal around `memory_stall_distribution` from side effect, use this article to order evidence before tuning.
...
- Source: `pressure`
- Field: `memory_stall_distribution`
- Domain: stall accumulation and backpressure
- Signal family: stall-time accumulation

## Operational Meaning (Pressure Lens)
This field is strongest for separating pressure stalled-time signal around `memory_stall_distribution` from side-effect noise when some/full trends diverge.
...
For `memory_stall_distribution`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `memory_stall_distribution`.
...
This field is a manifestation of **stall-time accumulation in kernel subsystems (memory_stall_distribution)**.

- Kernel path: runnable wait, memory reclaim stalls, I/O wait chains.
- Typical trigger (memory_stall_distribution): resource contention that does not always raise CPU%.
- Cross-check (memory_stall_distribution): runqueue, reclaim, and queue-depth signals.
...
### Decision Record
- Primary claim: pressure.memory_stall_distribution indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: memory_stall_distribution was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: memory_stall_distribution
...
### Anchor (Pressure)
Field under practice: memory_stall_distribution
...
- Field in focus: memory_stall_distribution
