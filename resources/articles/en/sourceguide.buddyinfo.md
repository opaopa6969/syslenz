# Source Guide: buddyinfo

## NAME
`sourceguide.buddyinfo` - source-level operational reading guide
...
`buddyinfo` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.buddyinfo) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat buddyinfo as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for buddyinfo.
...
### Decision Record
- Primary claim: sourceguide.buddyinfo indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: buddyinfo was treated as evidence in a chain, not a singleton verdict.
