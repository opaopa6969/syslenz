...
## NAME
`concept.reclaim-vs-cache` - foundational systems concept for diagnosis
...
## Core Thesis
`Reclaim vs Cache` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.reclaim-vs-cache) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat reclaim-vs-cache as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around reclaim-vs-cache.
...
### Decision Record
- Primary claim: concept.reclaim-vs-cache indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: reclaim-vs-cache was treated as evidence in a chain, not a singleton verdict.
...
- Field in focus: reclaim-vs-cache
