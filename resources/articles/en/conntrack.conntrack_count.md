...
## NAME
`conntrack.conntrack_count` - metric signal from `conntrack` (conntrack_count)
...
## Metric Snapshot
- ID: `conntrack.conntrack_count`
- Source: `conntrack`
- Field: `conntrack_count`
- Domain: connection state tracking
...
## Episode
A burst of short-lived flows exhausted table headroom before packet drop alarms fired.
...
3. Pair one sibling metric in `conntrack` plus one pressure/queue metric from another source.
...
This signal (conntrack.conntrack_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat conntrack_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: conntrack.conntrack_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: conntrack_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: conntrack_count
- Source anchor: conntrack
...
- Field in focus: conntrack_count
