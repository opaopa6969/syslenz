...
## NAME
`meminfo.Slab` - metric signal from `meminfo` (Slab)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `Slab` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `Slab` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `Slab` from side-effect noise when free/reclaimable trends diverge.
...
For `Slab`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Slab`.
...
This field is a manifestation of **memory accounting surfaces (Slab)**.

- Kernel path (Slab): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (Slab): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (Slab): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case B (`Slab`): Cross-source correlation reversed the initial diagnosis.

### Incident Slice 2 (MemInfo)
Case C (`Slab`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 3 (MemInfo)
Case A (`Slab`): First anomaly came from this field trend, not absolute value.
...
### Decision Record
- Primary claim: meminfo.Slab indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Slab was treated as evidence in a chain, not a singleton verdict.
