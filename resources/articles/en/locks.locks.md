...
## NAME
`locks.locks` - metric signal from `locks` (locks)
...
This signal (locks.locks) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: locks.locks indicated a meaningful state transition.
