...
## NAME
`vmstat.swap_ra` - metric signal from `vmstat` (swap_ra)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `swap_ra` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `swap_ra` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating vmstat reclaim signal around `swap_ra` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
Swap-related movement stayed low at first, then accelerated when cache headroom collapsed.

For `swap_ra`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `swap_ra`.
...
This field is a manifestation of **virtual memory state transitions (`swap_ra`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`swap_ra`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `swap_ra`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `swap_ra`
- T-5m: first vmstat reclaim or scan anomaly candidate around `swap_ra`
- T0: user symptom confirmed with vmstat-side context around `swap_ra`
- T+3m: first hypothesis written with vmstat reclaim assumption for `swap_ra`
- T+6m: cross-source validation including scheduler or writeback for `swap_ra`
- T+10m: mitigation applied with rollback guard in the vmstat path for `swap_ra`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `swap_ra`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `swap_ra`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `swap_ra`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `swap_ra`
...
### Decision Record
- Primary claim: vmstat.swap_ra indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: swap_ra was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `swap_ra`.
...
- Field anchor: swap_ra
- Source anchor: vmstat (`swap_ra`)
...
### Debrief Questions
- Which vmstat-side step around `swap_ra` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `swap_ra` versus side effect?
- What vmstat instrumentation change around `swap_ra` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: swap_ra

## Failure Archetype Matrix
- Archetype A (`swap_ra`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`swap_ra`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`swap_ra`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: swap_ra
