...
## NAME
`swaps.usage_pct` - metric signal from `swaps` (usage_pct)
...
This signal (swaps.usage_pct) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: swaps.usage_pct indicated a meaningful state transition.
