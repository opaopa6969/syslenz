...
## NAME
`meminfo.VmallocTotal` - metric signal from `meminfo` (VmallocTotal)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `VmallocTotal` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `VmallocTotal` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `VmallocTotal` from side-effect noise when free/reclaimable trends diverge.
...
For `VmallocTotal`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `VmallocTotal`.
...
This field is a manifestation of **memory accounting surfaces (VmallocTotal)**.

- Kernel path (VmallocTotal): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (VmallocTotal): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (VmallocTotal): vmstat reclaim counters and pressure metrics.
...
### Decision Record
- Primary claim: meminfo.VmallocTotal indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: VmallocTotal was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: VmallocTotal
