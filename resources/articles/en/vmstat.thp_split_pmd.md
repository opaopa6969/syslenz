...
## NAME
`vmstat.thp_split_pmd` - metric signal from `vmstat` (thp_split_pmd)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `thp_split_pmd` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `thp_split_pmd` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.thp_split_pmd`
- Source: `vmstat` (`thp_split_pmd`)
- Field: `thp_split_pmd`
- Domain: virtual memory and reclaim behavior
- Signal family: hugepage/compaction dynamics

## Operational Meaning (VMStat Lens)
This field is strongest for separating vmstat reclaim signal around `thp_split_pmd` from side-effect noise when scan and reclaim trends diverge.
...
For `thp_split_pmd`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `thp_split_pmd`.
...
This field is a manifestation of **virtual memory state transitions (`thp_split_pmd`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`thp_split_pmd`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `thp_split_pmd`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `thp_split_pmd`
- T-5m: first vmstat reclaim or scan anomaly candidate around `thp_split_pmd`
- T0: user symptom confirmed with vmstat-side context around `thp_split_pmd`
- T+3m: first hypothesis written with vmstat reclaim assumption for `thp_split_pmd`
- T+6m: cross-source validation including scheduler or writeback for `thp_split_pmd`
- T+10m: mitigation applied with rollback guard in the vmstat path for `thp_split_pmd`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `thp_split_pmd`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `thp_split_pmd`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `thp_split_pmd`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `thp_split_pmd`
...
### Decision Record
- Primary claim: vmstat.thp_split_pmd indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: thp_split_pmd was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `thp_split_pmd`.
...
- Field anchor: thp_split_pmd
- Source anchor: vmstat (`thp_split_pmd`)
...
### Debrief Questions
- Which vmstat-side step around `thp_split_pmd` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `thp_split_pmd` versus side effect?
- What vmstat instrumentation change around `thp_split_pmd` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: thp_split_pmd

## Failure Archetype Matrix
- Archetype A (`thp_split_pmd`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`thp_split_pmd`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`thp_split_pmd`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: thp_split_pmd
