...
## NAME
`version.compiler` - metric signal from `version` (compiler)
...
This signal (version.compiler) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat compiler as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: version.compiler indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: compiler was treated as evidence in a chain, not a singleton verdict.
