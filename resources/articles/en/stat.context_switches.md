...
## NAME
`stat.context_switches` - metric signal from `stat` (context_switches)
...
This signal (stat.context_switches) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat context_switches as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: stat.context_switches indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: context_switches was treated as evidence in a chain, not a singleton verdict.
...
- Field in focus: context_switches
