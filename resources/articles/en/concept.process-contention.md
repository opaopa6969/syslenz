...
## NAME
`concept.process-contention` - foundational systems concept for diagnosis
...
## Core Thesis
`Process Contention` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.process-contention) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat process-contention as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around process-contention.
...
### Decision Record
- Primary claim: concept.process-contention indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: process-contention was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: process-contention
...
- Field in focus: process-contention
