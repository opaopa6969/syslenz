...
## NAME
`file-nr.fd_allocated` - metric signal from `file-nr` (fd_allocated)
...
This signal (file-nr.fd_allocated) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat fd_allocated as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: file-nr.fd_allocated indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: fd_allocated was treated as evidence in a chain, not a singleton verdict.
