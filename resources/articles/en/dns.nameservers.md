...
## NAME
`dns.nameservers` - metric signal from `dns` (nameservers)
...
This signal (dns.nameservers) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat nameservers as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: dns.nameservers indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: nameservers was treated as evidence in a chain, not a singleton verdict.
