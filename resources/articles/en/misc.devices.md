...
## NAME
`misc.devices` - metric signal from `misc` (devices)
...
This signal (misc.devices) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: misc.devices indicated a meaningful state transition.
