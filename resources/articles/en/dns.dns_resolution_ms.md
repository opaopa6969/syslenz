...
## NAME
`dns.dns_resolution_ms` - metric signal from `dns` (dns_resolution_ms)
...
## Metric Snapshot
- ID: `dns.dns_resolution_ms`
- Source: `dns`
- Field: `dns_resolution_ms`
...
3. Pair one sibling metric in `dns` plus one pressure/queue metric from another source.
...
This signal (dns.dns_resolution_ms) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat dns_resolution_ms as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: dns.dns_resolution_ms indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: dns_resolution_ms was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: dns_resolution_ms
- Source anchor: dns
...
- Field in focus: dns_resolution_ms
