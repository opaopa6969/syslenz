...
## NAME
`schedstat.cpu_count` - metric signal from `schedstat` (cpu_count)
...
This signal (schedstat.cpu_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cpu_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: schedstat.cpu_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cpu_count was treated as evidence in a chain, not a singleton verdict.
