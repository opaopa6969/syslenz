...
## NAME
`concept.capacity-vs-utilization` - foundational systems concept for diagnosis
...
## Core Thesis
`Capacity vs Utilization` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.capacity-vs-utilization) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat capacity-vs-utilization as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around capacity-vs-utilization.
...
### Decision Record
- Primary claim: concept.capacity-vs-utilization indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: capacity-vs-utilization was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: capacity-vs-utilization
...
- Field in focus: capacity-vs-utilization
