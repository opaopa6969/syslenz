...
## NAME
`pressure.io_some_avg60` - metric signal from `pressure` (io_some_avg60)

## WHY NOW
Read this when pressure stalled-time behavior around `io_some_avg60` is noisy and you need a fast, defensible decision.
If you cannot tell pressure stall signal around `io_some_avg60` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating pressure stalled-time signal around `io_some_avg60` from side-effect noise when some/full trends diverge.
...
For `io_some_avg60`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `io_some_avg60`.
...
This field is a manifestation of **stall-time accumulation in kernel subsystems (io_some_avg60)**.

- Kernel path: runnable wait, memory reclaim stalls, I/O wait chains.
- Typical trigger (io_some_avg60): resource contention that does not always raise CPU%.
- Cross-check (io_some_avg60): runqueue, reclaim, and queue-depth signals.
...
### Decision Record
- Primary claim: pressure.io_some_avg60 indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: io_some_avg60 was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (Pressure)
Field under practice: io_some_avg60
