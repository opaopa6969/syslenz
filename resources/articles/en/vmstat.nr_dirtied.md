...
## NAME
`vmstat.nr_dirtied` - metric signal from `vmstat` (nr_dirtied)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `nr_dirtied` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `nr_dirtied` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.nr_dirtied`
- Source: `vmstat` (`nr_dirtied`)
- Field: `nr_dirtied`
...
This field is strongest for separating vmstat reclaim signal around `nr_dirtied` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `nr_dirtied`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `nr_dirtied`.
...
This field is a manifestation of **virtual memory state transitions (`nr_dirtied`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`nr_dirtied`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `nr_dirtied`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `nr_dirtied`
- T-5m: first vmstat reclaim or scan anomaly candidate around `nr_dirtied`
- T0: user symptom confirmed with vmstat-side context around `nr_dirtied`
- T+3m: first hypothesis written with vmstat reclaim assumption for `nr_dirtied`
- T+6m: cross-source validation including scheduler or writeback for `nr_dirtied`
- T+10m: mitigation applied with rollback guard in the vmstat path for `nr_dirtied`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `nr_dirtied`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `nr_dirtied`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `nr_dirtied`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `nr_dirtied`
...
### Decision Record
- Primary claim: vmstat.nr_dirtied indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: nr_dirtied was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `nr_dirtied`.
...
- Field anchor: nr_dirtied
- Source anchor: vmstat (`nr_dirtied`)
...
### Debrief Questions
- Which vmstat-side step around `nr_dirtied` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `nr_dirtied` versus side effect?
- What vmstat instrumentation change around `nr_dirtied` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: nr_dirtied

## Failure Archetype Matrix
- Archetype A (`nr_dirtied`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`nr_dirtied`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`nr_dirtied`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: nr_dirtied
