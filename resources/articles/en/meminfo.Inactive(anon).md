...
## NAME
`meminfo.Inactive(anon)` - metric signal from `meminfo` (Inactive(anon))

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `Inactive(anon)` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `Inactive(anon)` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `Inactive(anon)` from side-effect noise when free/reclaimable trends diverge.
...
For `Inactive(anon)`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Inactive(anon)`.
...
This field is a manifestation of **memory accounting surfaces (Inactive(anon))**.

- Kernel path (Inactive(anon)): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (Inactive(anon)): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (Inactive(anon)): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case B (`Inactive(anon)`): Cross-source correlation reversed the initial diagnosis.

### Incident Slice 2 (MemInfo)
Case C (`Inactive(anon)`): Reversible mitigation provided faster learning than invasive change.

### Incident Slice 3 (MemInfo)
Case A (`Inactive(anon)`): First anomaly came from this field trend, not absolute value.
...
### Decision Record
- Primary claim: meminfo.Inactive(anon) indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Inactive(anon) was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: Inactive(anon)
