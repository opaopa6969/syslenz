...
## NAME
`sourceguide.crypto` - source-level operational reading guide
...
`crypto` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.crypto) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat crypto as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for crypto.
...
### Decision Record
- Primary claim: sourceguide.crypto indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: crypto was treated as evidence in a chain, not a singleton verdict.
