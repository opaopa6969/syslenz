...
## NAME
`version.kernel_version` - metric signal from `version` (kernel_version)
...
3. Pair one sibling metric in `version` plus one pressure/queue metric from another source.
...
This signal (version.kernel_version) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat kernel_version as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: version.kernel_version indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: kernel_version was treated as evidence in a chain, not a singleton verdict.
