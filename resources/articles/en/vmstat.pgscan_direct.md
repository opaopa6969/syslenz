...
## NAME
`vmstat.pgscan_direct` - metric signal from `vmstat` (pgscan_direct)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pgscan_direct` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pgscan_direct` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.pgscan_direct`
- Source: `vmstat` (`pgscan_direct`)
- Field: `pgscan_direct`
...
This field is strongest for separating vmstat reclaim signal around `pgscan_direct` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
During a latency spike, reclaim-related counters moved before user-visible error rate changed.

For `pgscan_direct`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pgscan_direct`.
...
This field is a manifestation of **virtual memory state transitions (`pgscan_direct`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pgscan_direct`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pgscan_direct`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pgscan_direct`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pgscan_direct`
- T0: user symptom confirmed with vmstat-side context around `pgscan_direct`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pgscan_direct`
- T+6m: cross-source validation including scheduler or writeback for `pgscan_direct`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pgscan_direct`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pgscan_direct`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pgscan_direct`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pgscan_direct`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pgscan_direct`
...
### Decision Record
- Primary claim: vmstat.pgscan_direct indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pgscan_direct was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pgscan_direct`.
...
- Field anchor: pgscan_direct
- Source anchor: vmstat (`pgscan_direct`)
...
### Debrief Questions
- Which vmstat-side step around `pgscan_direct` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pgscan_direct` versus side effect?
- What vmstat instrumentation change around `pgscan_direct` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pgscan_direct

## Failure Archetype Matrix
- Archetype A (`pgscan_direct`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pgscan_direct`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pgscan_direct`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pgscan_direct
