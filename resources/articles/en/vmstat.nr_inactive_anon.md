...
## NAME
`vmstat.nr_inactive_anon` - metric signal from `vmstat` (nr_inactive_anon)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `nr_inactive_anon` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `nr_inactive_anon` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.nr_inactive_anon`
- Source: `vmstat` (`nr_inactive_anon`)
...
This field is strongest for separating vmstat reclaim signal around `nr_inactive_anon` from side-effect noise when scan and reclaim trends diverge.
...
For `nr_inactive_anon`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `nr_inactive_anon`.
...
This field is a manifestation of **virtual memory state transitions (`nr_inactive_anon`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`nr_inactive_anon`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `nr_inactive_anon`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `nr_inactive_anon`
- T-5m: first vmstat reclaim or scan anomaly candidate around `nr_inactive_anon`
- T0: user symptom confirmed with vmstat-side context around `nr_inactive_anon`
- T+3m: first hypothesis written with vmstat reclaim assumption for `nr_inactive_anon`
- T+6m: cross-source validation including scheduler or writeback for `nr_inactive_anon`
- T+10m: mitigation applied with rollback guard in the vmstat path for `nr_inactive_anon`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `nr_inactive_anon`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `nr_inactive_anon`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `nr_inactive_anon`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `nr_inactive_anon`
...
### Decision Record
- Primary claim: vmstat.nr_inactive_anon indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: nr_inactive_anon was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `nr_inactive_anon`.
...
- Field anchor: nr_inactive_anon
- Source anchor: vmstat (`nr_inactive_anon`)
...
### Debrief Questions
- Which vmstat-side step around `nr_inactive_anon` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `nr_inactive_anon` versus side effect?
- What vmstat instrumentation change around `nr_inactive_anon` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: nr_inactive_anon

## Failure Archetype Matrix
- Archetype A (`nr_inactive_anon`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`nr_inactive_anon`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`nr_inactive_anon`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: nr_inactive_anon
