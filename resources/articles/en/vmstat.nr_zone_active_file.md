...
## NAME
`vmstat.nr_zone_active_file` - metric signal from `vmstat` (nr_zone_active_file)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `nr_zone_active_file` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `nr_zone_active_file` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.nr_zone_active_file`
- Source: `vmstat` (`nr_zone_active_file`)
...
This field is strongest for separating vmstat reclaim signal around `nr_zone_active_file` from side-effect noise when scan and reclaim trends diverge.
...
For `nr_zone_active_file`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `nr_zone_active_file`.
...
This field is a manifestation of **virtual memory state transitions (`nr_zone_active_file`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`nr_zone_active_file`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `nr_zone_active_file`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `nr_zone_active_file`
- T-5m: first vmstat reclaim or scan anomaly candidate around `nr_zone_active_file`
- T0: user symptom confirmed with vmstat-side context around `nr_zone_active_file`
- T+3m: first hypothesis written with vmstat reclaim assumption for `nr_zone_active_file`
- T+6m: cross-source validation including scheduler or writeback for `nr_zone_active_file`
- T+10m: mitigation applied with rollback guard in the vmstat path for `nr_zone_active_file`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `nr_zone_active_file`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `nr_zone_active_file`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `nr_zone_active_file`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `nr_zone_active_file`
...
### Decision Record
- Primary claim: vmstat.nr_zone_active_file indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: nr_zone_active_file was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `nr_zone_active_file`.
...
- Field anchor: nr_zone_active_file
- Source anchor: vmstat (`nr_zone_active_file`)
...
### Debrief Questions
- Which vmstat-side step around `nr_zone_active_file` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `nr_zone_active_file` versus side effect?
- What vmstat instrumentation change around `nr_zone_active_file` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: nr_zone_active_file

## Failure Archetype Matrix
- Archetype A (`nr_zone_active_file`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`nr_zone_active_file`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`nr_zone_active_file`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: nr_zone_active_file
