...
## NAME
`devices.device_count` - metric signal from `devices` (device_count)
...
3. Pair one sibling metric in `devices` plus one pressure/queue metric from another source.
...
This signal (devices.device_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat device_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: devices.device_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: device_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: device_count
- Source anchor: devices
