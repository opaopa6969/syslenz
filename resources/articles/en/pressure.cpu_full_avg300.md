...
## NAME
`pressure.cpu_full_avg300` - metric signal from `pressure` (cpu_full_avg300)

## WHY NOW
Read this when pressure stalled-time behavior around `cpu_full_avg300` is noisy and you need a fast, defensible decision.
If you cannot tell pressure stall signal around `cpu_full_avg300` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `pressure.cpu_full_avg300`
- Source: `pressure`
- Field: `cpu_full_avg300`
- Domain: stall accumulation and backpressure
- Signal family: stall-time accumulation

## Operational Meaning (Pressure Lens)
This field is strongest for separating pressure stalled-time signal around `cpu_full_avg300` from side-effect noise when some/full trends diverge.
...
For `cpu_full_avg300`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `cpu_full_avg300`.
...
This field is a manifestation of **stall-time accumulation in kernel subsystems (cpu_full_avg300)**.

- Kernel path: runnable wait, memory reclaim stalls, I/O wait chains.
- Typical trigger (cpu_full_avg300): resource contention that does not always raise CPU%.
- Cross-check (cpu_full_avg300): runqueue, reclaim, and queue-depth signals.
...
### Decision Record
- Primary claim: pressure.cpu_full_avg300 indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cpu_full_avg300 was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (Pressure)
Field under practice: cpu_full_avg300
...
- Field in focus: cpu_full_avg300
