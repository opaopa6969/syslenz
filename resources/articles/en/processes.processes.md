...
## NAME
`processes.processes` - metric signal from `processes` (processes)
...
This signal (processes.processes) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: processes.processes indicated a meaningful state transition.
