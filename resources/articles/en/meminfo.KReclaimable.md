...
## NAME
`meminfo.KReclaimable` - metric signal from `meminfo` (KReclaimable)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `KReclaimable` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `KReclaimable` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `KReclaimable` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
During a latency spike, reclaim-related counters moved before user-visible error rate changed.

For `KReclaimable`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `KReclaimable`.
...
This field is a manifestation of **memory accounting surfaces (KReclaimable)**.

- Kernel path (KReclaimable): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (KReclaimable): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (KReclaimable): vmstat reclaim counters and pressure metrics.
...
### Decision Record
- Primary claim: meminfo.KReclaimable indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: KReclaimable was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: KReclaimable
