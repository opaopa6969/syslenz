...
## NAME
`ss.tcp_timewait` - metric signal from `ss` (tcp_timewait)
...
This signal (ss.tcp_timewait) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat tcp_timewait as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: ss.tcp_timewait indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: tcp_timewait was treated as evidence in a chain, not a singleton verdict.
