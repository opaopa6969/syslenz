...
## NAME
`stat.procs_running` - metric signal from `stat` (procs_running)
...
This signal (stat.procs_running) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat procs_running as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: stat.procs_running indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: procs_running was treated as evidence in a chain, not a singleton verdict.
