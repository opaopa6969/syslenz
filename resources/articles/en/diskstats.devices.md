...
## NAME
`diskstats.devices` - metric signal from `diskstats` (devices)
...
For `devices`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `devices`.
...
### Decision Record
- Primary claim: diskstats.devices indicated a meaningful state transition.
