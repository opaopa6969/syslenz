...
## NAME
`cmdline.param_count` - metric signal from `cmdline` (param_count)
...
3. Pair one sibling metric in `cmdline` plus one pressure/queue metric from another source.
...
This signal (cmdline.param_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat param_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: cmdline.param_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: param_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: param_count
- Source anchor: cmdline
