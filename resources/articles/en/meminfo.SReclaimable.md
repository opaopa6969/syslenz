...
## NAME
`meminfo.SReclaimable` - metric signal from `meminfo` (SReclaimable)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `SReclaimable` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `SReclaimable` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `SReclaimable` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
During a latency spike, reclaim-related counters moved before user-visible error rate changed.

For `SReclaimable`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `SReclaimable`.
...
This field is a manifestation of **memory accounting surfaces (SReclaimable)**.

- Kernel path (SReclaimable): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (SReclaimable): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (SReclaimable): vmstat reclaim counters and pressure metrics.
...
### Decision Record
- Primary claim: meminfo.SReclaimable indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: SReclaimable was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: SReclaimable
