# Source Guide: conntrack

## NAME
`sourceguide.conntrack` - source-level operational reading guide
...
`conntrack` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.conntrack) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat conntrack as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for conntrack.
...
### Decision Record
- Primary claim: sourceguide.conntrack indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: conntrack was treated as evidence in a chain, not a singleton verdict.
