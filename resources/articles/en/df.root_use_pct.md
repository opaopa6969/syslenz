...
## NAME
`df.root_use_pct` - metric signal from `df` (root_use_pct)
...
This signal (df.root_use_pct) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat root_use_pct as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: df.root_use_pct indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: root_use_pct was treated as evidence in a chain, not a singleton verdict.
