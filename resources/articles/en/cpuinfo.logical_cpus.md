...
## NAME
`cpuinfo.logical_cpus` - metric signal from `cpuinfo` (logical_cpus)
...
This signal (cpuinfo.logical_cpus) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat logical_cpus as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: cpuinfo.logical_cpus indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: logical_cpus was treated as evidence in a chain, not a singleton verdict.
