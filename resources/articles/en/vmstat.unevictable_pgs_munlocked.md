...
## NAME
`vmstat.unevictable_pgs_munlocked` - metric signal from `vmstat` (unevictable_pgs_munlocked)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `unevictable_pgs_munlocked` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `unevictable_pgs_munlocked` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.unevictable_pgs_munlocked`
- Source: `vmstat` (`unevictable_pgs_munlocked`)
...
This field is strongest for separating vmstat reclaim signal around `unevictable_pgs_munlocked` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `unevictable_pgs_munlocked`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `unevictable_pgs_munlocked`.
...
This field is a manifestation of **virtual memory state transitions (`unevictable_pgs_munlocked`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`unevictable_pgs_munlocked`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `unevictable_pgs_munlocked`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `unevictable_pgs_munlocked`
- T-5m: first vmstat reclaim or scan anomaly candidate around `unevictable_pgs_munlocked`
- T0: user symptom confirmed with vmstat-side context around `unevictable_pgs_munlocked`
- T+3m: first hypothesis written with vmstat reclaim assumption for `unevictable_pgs_munlocked`
- T+6m: cross-source validation including scheduler or writeback for `unevictable_pgs_munlocked`
- T+10m: mitigation applied with rollback guard in the vmstat path for `unevictable_pgs_munlocked`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `unevictable_pgs_munlocked`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `unevictable_pgs_munlocked`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `unevictable_pgs_munlocked`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `unevictable_pgs_munlocked`
...
### Decision Record
- Primary claim: vmstat.unevictable_pgs_munlocked indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: unevictable_pgs_munlocked was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `unevictable_pgs_munlocked`.
...
- Field anchor: unevictable_pgs_munlocked
- Source anchor: vmstat (`unevictable_pgs_munlocked`)
...
### Debrief Questions
- Which vmstat-side step around `unevictable_pgs_munlocked` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `unevictable_pgs_munlocked` versus side effect?
- What vmstat instrumentation change around `unevictable_pgs_munlocked` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: unevictable_pgs_munlocked

## Failure Archetype Matrix
- Archetype A (`unevictable_pgs_munlocked`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`unevictable_pgs_munlocked`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`unevictable_pgs_munlocked`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: unevictable_pgs_munlocked
