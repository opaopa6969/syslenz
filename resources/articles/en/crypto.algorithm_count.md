...
## NAME
`crypto.algorithm_count` - metric signal from `crypto` (algorithm_count)
...
## Metric Snapshot
- ID: `crypto.algorithm_count`
- Source: `crypto`
- Field: `algorithm_count`
...
3. Pair one sibling metric in `crypto` plus one pressure/queue metric from another source.
...
This signal (crypto.algorithm_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat algorithm_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: crypto.algorithm_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: algorithm_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: algorithm_count
- Source anchor: crypto
...
- Field in focus: algorithm_count
