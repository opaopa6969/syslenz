...
## NAME
`concept.reading-metrics` - foundational systems concept for diagnosis
...
## Core Thesis
`How to Read Metrics` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.reading-metrics) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat reading-metrics as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around reading-metrics.
...
### Decision Record
- Primary claim: concept.reading-metrics indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: reading-metrics was treated as evidence in a chain, not a singleton verdict.
...
- Field in focus: reading-metrics
