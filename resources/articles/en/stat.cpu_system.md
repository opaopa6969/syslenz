...
## NAME
`stat.cpu_system` - metric signal from `stat` (cpu_system)
...
This signal (stat.cpu_system) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cpu_system as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: stat.cpu_system indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cpu_system was treated as evidence in a chain, not a singleton verdict.
