...
## NAME
`meminfo.DirectMap4k` - metric signal from `meminfo` (DirectMap4k)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `DirectMap4k` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `DirectMap4k` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `DirectMap4k` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `DirectMap4k`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `DirectMap4k`.
...
This field is a manifestation of **memory accounting surfaces (DirectMap4k)**.

- Kernel path (DirectMap4k): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (DirectMap4k): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (DirectMap4k): vmstat reclaim counters and pressure metrics.

## Casebook (MemInfo Family)

### Incident Slice 1 (MemInfo)
Case A (`DirectMap4k`): First anomaly came from this field trend, not absolute value.

### Incident Slice 2 (MemInfo)
Case B (`DirectMap4k`): Cross-source correlation reversed the initial diagnosis.

### Incident Slice 3 (MemInfo)
Case C (`DirectMap4k`): Reversible mitigation provided faster learning than invasive change.
...
### Decision Record
- Primary claim: meminfo.DirectMap4k indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: DirectMap4k was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: DirectMap4k
