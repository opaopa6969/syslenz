...
## NAME
`buddyinfo.zone_count` - metric signal from `buddyinfo` (zone_count)
...
3. Pair one sibling metric in `buddyinfo` plus one pressure/queue metric from another source.
...
This signal (buddyinfo.zone_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat zone_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: buddyinfo.zone_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: zone_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: zone_count
- Source anchor: buddyinfo
