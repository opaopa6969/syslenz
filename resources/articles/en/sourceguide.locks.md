...
## NAME
`sourceguide.locks` - source-level operational reading guide
...
`locks` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.locks) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat locks as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for locks.
...
### Decision Record
- Primary claim: sourceguide.locks indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: locks was treated as evidence in a chain, not a singleton verdict.
