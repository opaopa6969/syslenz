...
## NAME
`swaps.total_size` - metric signal from `swaps` (total_size)
...
3. Pair one sibling metric in `swaps` plus one pressure/queue metric from another source.
...
This signal (swaps.total_size) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat total_size as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: swaps.total_size indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: total_size was treated as evidence in a chain, not a singleton verdict.
