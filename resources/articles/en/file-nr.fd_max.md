...
## NAME
`file-nr.fd_max` - metric signal from `file-nr` (fd_max)
...
This signal (file-nr.fd_max) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat fd_max as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: file-nr.fd_max indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: fd_max was treated as evidence in a chain, not a singleton verdict.
