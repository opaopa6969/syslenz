...
## NAME
`vmstat.compact_success` - metric signal from `vmstat` (compact_success)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `compact_success` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `compact_success` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.compact_success`
- Source: `vmstat` (`compact_success`)
...
This field is strongest for separating vmstat reclaim signal around `compact_success` from side-effect noise when scan and reclaim trends diverge.
...
For `compact_success`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `compact_success`.
...
This field is a manifestation of **virtual memory state transitions (`compact_success`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`compact_success`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `compact_success`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `compact_success`
- T-5m: first vmstat reclaim or scan anomaly candidate around `compact_success`
- T0: user symptom confirmed with vmstat-side context around `compact_success`
- T+3m: first hypothesis written with vmstat reclaim assumption for `compact_success`
- T+6m: cross-source validation including scheduler or writeback for `compact_success`
- T+10m: mitigation applied with rollback guard in the vmstat path for `compact_success`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `compact_success`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `compact_success`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `compact_success`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `compact_success`
...
### Decision Record
- Primary claim: vmstat.compact_success indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: compact_success was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `compact_success`.
...
- Field anchor: compact_success
- Source anchor: vmstat (`compact_success`)
...
### Debrief Questions
- Which vmstat-side step around `compact_success` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `compact_success` versus side effect?
- What vmstat instrumentation change around `compact_success` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: compact_success

## Failure Archetype Matrix
- Archetype A (`compact_success`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`compact_success`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`compact_success`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: compact_success
