...
## NAME
`zoneinfo.zone_count` - metric signal from `zoneinfo` (zone_count)
...
3. Pair one sibling metric in `zoneinfo` plus one pressure/queue metric from another source.
...
This signal (zoneinfo.zone_count) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: zoneinfo.zone_count indicated a meaningful state transition.
