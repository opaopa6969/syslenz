...
## NAME
`df.available_disk` - metric signal from `df` (available_disk)
...
This signal (df.available_disk) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat available_disk as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: df.available_disk indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: available_disk was treated as evidence in a chain, not a singleton verdict.
