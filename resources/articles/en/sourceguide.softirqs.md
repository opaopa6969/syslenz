# Source Guide: softirqs

## NAME
`sourceguide.softirqs` - source-level operational reading guide
...
`softirqs` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.softirqs) is not only a number; it is an exposed edge of kernel state transitions.
...
### Focus
Source-level operational literacy for softirqs.
...
### Decision Record
- Primary claim: sourceguide.softirqs indicated a meaningful state transition.
