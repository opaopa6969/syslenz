...
## NAME
`vmstat.pgskip_movable` - metric signal from `vmstat` (pgskip_movable)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pgskip_movable` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pgskip_movable` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.pgskip_movable`
- Source: `vmstat` (`pgskip_movable`)
- Field: `pgskip_movable`
...
This field is strongest for separating vmstat reclaim signal around `pgskip_movable` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `pgskip_movable`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pgskip_movable`.
...
This field is a manifestation of **virtual memory state transitions (`pgskip_movable`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pgskip_movable`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pgskip_movable`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pgskip_movable`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pgskip_movable`
- T0: user symptom confirmed with vmstat-side context around `pgskip_movable`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pgskip_movable`
- T+6m: cross-source validation including scheduler or writeback for `pgskip_movable`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pgskip_movable`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pgskip_movable`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pgskip_movable`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pgskip_movable`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pgskip_movable`
...
### Decision Record
- Primary claim: vmstat.pgskip_movable indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pgskip_movable was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pgskip_movable`.
...
- Field anchor: pgskip_movable
- Source anchor: vmstat (`pgskip_movable`)
...
### Debrief Questions
- Which vmstat-side step around `pgskip_movable` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pgskip_movable` versus side effect?
- What vmstat instrumentation change around `pgskip_movable` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pgskip_movable

## Failure Archetype Matrix
- Archetype A (`pgskip_movable`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pgskip_movable`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pgskip_movable`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pgskip_movable
