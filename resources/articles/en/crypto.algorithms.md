...
## NAME
`crypto.algorithms` - metric signal from `crypto` (algorithms)
...
This signal (crypto.algorithms) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat algorithms as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: crypto.algorithms indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: algorithms was treated as evidence in a chain, not a singleton verdict.
