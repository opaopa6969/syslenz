...
## NAME
`vmstat.allocstall_movable` - metric signal from `vmstat` (allocstall_movable)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `allocstall_movable` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `allocstall_movable` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.allocstall_movable`
- Source: `vmstat` (`allocstall_movable`)
...
This field is strongest for separating vmstat reclaim signal around `allocstall_movable` from side-effect noise when scan and reclaim trends diverge.
...
For `allocstall_movable`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `allocstall_movable`.
...
This field is a manifestation of **virtual memory state transitions (`allocstall_movable`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`allocstall_movable`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `allocstall_movable`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `allocstall_movable`
- T-5m: first vmstat reclaim or scan anomaly candidate around `allocstall_movable`
- T0: user symptom confirmed with vmstat-side context around `allocstall_movable`
- T+3m: first hypothesis written with vmstat reclaim assumption for `allocstall_movable`
- T+6m: cross-source validation including scheduler or writeback for `allocstall_movable`
- T+10m: mitigation applied with rollback guard in the vmstat path for `allocstall_movable`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `allocstall_movable`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `allocstall_movable`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `allocstall_movable`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `allocstall_movable`
...
### Decision Record
- Primary claim: vmstat.allocstall_movable indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: allocstall_movable was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `allocstall_movable`.
...
- Field anchor: allocstall_movable
- Source anchor: vmstat (`allocstall_movable`)
...
### Debrief Questions
- Which vmstat-side step around `allocstall_movable` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `allocstall_movable` versus side effect?
- What vmstat instrumentation change around `allocstall_movable` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: allocstall_movable

## Failure Archetype Matrix
- Archetype A (`allocstall_movable`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`allocstall_movable`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`allocstall_movable`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: allocstall_movable
