...
## NAME
`interrupts.interrupts` - metric signal from `interrupts` (interrupts)
...
This signal (interrupts.interrupts) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: interrupts.interrupts indicated a meaningful state transition.
