...
## NAME
`loadavg.total_threads` - metric signal from `loadavg` (total_threads)
...
This signal (loadavg.total_threads) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat total_threads as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: loadavg.total_threads indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: total_threads was treated as evidence in a chain, not a singleton verdict.
