...
## NAME
`vmstat.pswpin` - metric signal from `vmstat` (pswpin)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pswpin` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pswpin` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating vmstat reclaim signal around `pswpin` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
Swap-related movement stayed low at first, then accelerated when cache headroom collapsed.

For `pswpin`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pswpin`.
...
This field is a manifestation of **virtual memory state transitions (`pswpin`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pswpin`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pswpin`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pswpin`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pswpin`
- T0: user symptom confirmed with vmstat-side context around `pswpin`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pswpin`
- T+6m: cross-source validation including scheduler or writeback for `pswpin`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pswpin`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pswpin`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pswpin`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pswpin`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pswpin`
...
### Decision Record
- Primary claim: vmstat.pswpin indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pswpin was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pswpin`.
...
### Debrief Questions
- Which vmstat-side step around `pswpin` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pswpin` versus side effect?
- What vmstat instrumentation change around `pswpin` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pswpin

## Failure Archetype Matrix
- Archetype A (`pswpin`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pswpin`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pswpin`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pswpin
