...
## NAME
`vmstat.direct_map_level2_splits` - metric signal from `vmstat` (direct_map_level2_splits)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `direct_map_level2_splits` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `direct_map_level2_splits` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.direct_map_level2_splits`
- Source: `vmstat` (`direct_map_level2_splits`)
- Field: `direct_map_level2_splits`
...
This field is strongest for separating vmstat reclaim signal around `direct_map_level2_splits` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `direct_map_level2_splits`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `direct_map_level2_splits`.
...
This field is a manifestation of **virtual memory state transitions (`direct_map_level2_splits`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`direct_map_level2_splits`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `direct_map_level2_splits`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `direct_map_level2_splits`
- T-5m: first vmstat reclaim or scan anomaly candidate around `direct_map_level2_splits`
- T0: user symptom confirmed with vmstat-side context around `direct_map_level2_splits`
- T+3m: first hypothesis written with vmstat reclaim assumption for `direct_map_level2_splits`
- T+6m: cross-source validation including scheduler or writeback for `direct_map_level2_splits`
- T+10m: mitigation applied with rollback guard in the vmstat path for `direct_map_level2_splits`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `direct_map_level2_splits`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `direct_map_level2_splits`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `direct_map_level2_splits`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `direct_map_level2_splits`
...
### Decision Record
- Primary claim: vmstat.direct_map_level2_splits indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: direct_map_level2_splits was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `direct_map_level2_splits`.
...
- Field anchor: direct_map_level2_splits
- Source anchor: vmstat (`direct_map_level2_splits`)
...
### Debrief Questions
- Which vmstat-side step around `direct_map_level2_splits` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `direct_map_level2_splits` versus side effect?
- What vmstat instrumentation change around `direct_map_level2_splits` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: direct_map_level2_splits

## Failure Archetype Matrix
- Archetype A (`direct_map_level2_splits`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`direct_map_level2_splits`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`direct_map_level2_splits`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: direct_map_level2_splits
