...
## NAME
`proc/stat.context_switch_distribution` - metric signal from `proc/stat` (context_switch_distribution)
...
3. Pair one sibling metric in `proc/stat` plus one pressure/queue metric from another source.
...
This signal (proc/stat.context_switch_distribution) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: proc/stat.context_switch_distribution indicated a meaningful state transition.
