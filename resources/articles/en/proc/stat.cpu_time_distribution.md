...
## NAME
`proc/stat.cpu_time_distribution` - metric signal from `proc/stat` (cpu_time_distribution)
...
This signal (proc/stat.cpu_time_distribution) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cpu_time_distribution as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: proc/stat.cpu_time_distribution indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cpu_time_distribution was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: cpu_time_distribution
...
- Field in focus: cpu_time_distribution
