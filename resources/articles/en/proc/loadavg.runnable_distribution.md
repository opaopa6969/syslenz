# runnable_distribution

## NAME
`proc/loadavg.runnable_distribution` - metric signal from `proc/loadavg` (runnable_distribution)
...
3. Pair one sibling metric in `proc/loadavg` plus one pressure/queue metric from another source.
...
This signal (proc/loadavg.runnable_distribution) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat runnable_distribution as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: proc/loadavg.runnable_distribution indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: runnable_distribution was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: runnable_distribution
- Source anchor: proc/loadavg
...
- Field in focus: runnable_distribution
