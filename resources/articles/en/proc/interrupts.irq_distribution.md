...
## NAME
`proc/interrupts.irq_distribution` - metric signal from `proc/interrupts` (irq_distribution)
...
3. Pair one sibling metric in `proc/interrupts` plus one pressure/queue metric from another source.
...
This signal (proc/interrupts.irq_distribution) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat irq_distribution as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: proc/interrupts.irq_distribution indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: irq_distribution was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: irq_distribution
- Source anchor: proc/interrupts
...
- Field in focus: irq_distribution
