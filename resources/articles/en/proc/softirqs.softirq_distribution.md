# softirq_distribution

## NAME
`proc/softirqs.softirq_distribution` - metric signal from `proc/softirqs` (softirq_distribution)
...
3. Pair one sibling metric in `proc/softirqs` plus one pressure/queue metric from another source.
...
This signal (proc/softirqs.softirq_distribution) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat softirq_distribution as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: proc/softirqs.softirq_distribution indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: softirq_distribution was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: softirq_distribution
- Source anchor: proc/softirqs
...
- Field in focus: softirq_distribution
