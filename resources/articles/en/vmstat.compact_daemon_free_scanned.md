...
## NAME
`vmstat.compact_daemon_free_scanned` - metric signal from `vmstat` (compact_daemon_free_scanned)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `compact_daemon_free_scanned` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `compact_daemon_free_scanned` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.compact_daemon_free_scanned`
- Source: `vmstat` (`compact_daemon_free_scanned`)
...
This field is strongest for separating vmstat reclaim signal around `compact_daemon_free_scanned` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
During a latency spike, reclaim-related counters moved before user-visible error rate changed.

For `compact_daemon_free_scanned`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `compact_daemon_free_scanned`.
...
This field is a manifestation of **virtual memory state transitions (`compact_daemon_free_scanned`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`compact_daemon_free_scanned`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `compact_daemon_free_scanned`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `compact_daemon_free_scanned`
- T-5m: first vmstat reclaim or scan anomaly candidate around `compact_daemon_free_scanned`
- T0: user symptom confirmed with vmstat-side context around `compact_daemon_free_scanned`
- T+3m: first hypothesis written with vmstat reclaim assumption for `compact_daemon_free_scanned`
- T+6m: cross-source validation including scheduler or writeback for `compact_daemon_free_scanned`
- T+10m: mitigation applied with rollback guard in the vmstat path for `compact_daemon_free_scanned`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `compact_daemon_free_scanned`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `compact_daemon_free_scanned`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `compact_daemon_free_scanned`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `compact_daemon_free_scanned`
...
### Decision Record
- Primary claim: vmstat.compact_daemon_free_scanned indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: compact_daemon_free_scanned was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `compact_daemon_free_scanned`.
...
- Field anchor: compact_daemon_free_scanned
- Source anchor: vmstat (`compact_daemon_free_scanned`)
...
### Debrief Questions
- Which vmstat-side step around `compact_daemon_free_scanned` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `compact_daemon_free_scanned` versus side effect?
- What vmstat instrumentation change around `compact_daemon_free_scanned` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: compact_daemon_free_scanned

## Failure Archetype Matrix
- Archetype A (`compact_daemon_free_scanned`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`compact_daemon_free_scanned`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`compact_daemon_free_scanned`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: compact_daemon_free_scanned
