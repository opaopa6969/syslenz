...
## NAME
`modules.modules` - metric signal from `modules` (modules)
...
This signal (modules.modules) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: modules.modules indicated a meaningful state transition.
