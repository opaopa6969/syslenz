...
## NAME
`sourceguide.dns` - source-level operational reading guide
...
`dns` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.dns) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat dns as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for dns.
...
### Decision Record
- Primary claim: sourceguide.dns indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: dns was treated as evidence in a chain, not a singleton verdict.
