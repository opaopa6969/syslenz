...
## NAME
`swaps.total_used` - metric signal from `swaps` (total_used)
...
This signal (swaps.total_used) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat total_used as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: swaps.total_used indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: total_used was treated as evidence in a chain, not a singleton verdict.
