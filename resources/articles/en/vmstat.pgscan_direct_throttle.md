...
## NAME
`vmstat.pgscan_direct_throttle` - metric signal from `vmstat` (pgscan_direct_throttle)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pgscan_direct_throttle` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pgscan_direct_throttle` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.pgscan_direct_throttle`
- Source: `vmstat` (`pgscan_direct_throttle`)
...
This field is strongest for separating vmstat reclaim signal around `pgscan_direct_throttle` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
During a latency spike, reclaim-related counters moved before user-visible error rate changed.

For `pgscan_direct_throttle`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pgscan_direct_throttle`.
...
This field is a manifestation of **virtual memory state transitions (`pgscan_direct_throttle`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pgscan_direct_throttle`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pgscan_direct_throttle`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pgscan_direct_throttle`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pgscan_direct_throttle`
- T0: user symptom confirmed with vmstat-side context around `pgscan_direct_throttle`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pgscan_direct_throttle`
- T+6m: cross-source validation including scheduler or writeback for `pgscan_direct_throttle`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pgscan_direct_throttle`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pgscan_direct_throttle`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pgscan_direct_throttle`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pgscan_direct_throttle`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pgscan_direct_throttle`
...
### Decision Record
- Primary claim: vmstat.pgscan_direct_throttle indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pgscan_direct_throttle was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pgscan_direct_throttle`.
...
- Field anchor: pgscan_direct_throttle
- Source anchor: vmstat (`pgscan_direct_throttle`)
...
### Debrief Questions
- Which vmstat-side step around `pgscan_direct_throttle` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pgscan_direct_throttle` versus side effect?
- What vmstat instrumentation change around `pgscan_direct_throttle` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pgscan_direct_throttle

## Failure Archetype Matrix
- Archetype A (`pgscan_direct_throttle`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pgscan_direct_throttle`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pgscan_direct_throttle`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pgscan_direct_throttle
