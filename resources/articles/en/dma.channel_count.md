...
## NAME
`dma.channel_count` - metric signal from `dma` (channel_count)
...
3. Pair one sibling metric in `dma` plus one pressure/queue metric from another source.
...
This signal (dma.channel_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat channel_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: dma.channel_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: channel_count was treated as evidence in a chain, not a singleton verdict.
