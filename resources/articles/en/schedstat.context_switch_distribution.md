# context_switch_distribution

## NAME
`schedstat.context_switch_distribution` - metric signal from `schedstat` (context_switch_distribution)
...
- Source: `schedstat`
- Field: `context_switch_distribution`
- Domain: runqueue and scheduling
...
## Episode
One CPU group starved while global CPU looked fine; queue skew told the truth.
...
3. Pair one sibling metric in `schedstat` plus one pressure/queue metric from another source.
...
This field is a manifestation of **CFS/runqueue internals visible at user level**.

- Kernel path: enqueue/dequeue, fairness balancing, CPU-local queue behavior.
- Typical trigger: uneven workload pinning or bursty task creation.
- Cross-check: stat context switches and pressure CPU signals.

## Systems Narrative (Process)

This signal (schedstat.context_switch_distribution) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat context_switch_distribution as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: schedstat.context_switch_distribution indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: context_switch_distribution was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: context_switch_distribution
- Source anchor: schedstat
...
- Field in focus: context_switch_distribution
