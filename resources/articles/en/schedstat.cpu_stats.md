...
## NAME
`schedstat.cpu_stats` - metric signal from `schedstat` (cpu_stats)
...
This signal (schedstat.cpu_stats) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cpu_stats as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: schedstat.cpu_stats indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cpu_stats was treated as evidence in a chain, not a singleton verdict.
