...
## NAME
`ioports.region_count` - metric signal from `ioports` (region_count)
...
3. Pair one sibling metric in `ioports` plus one pressure/queue metric from another source.
...
This signal (ioports.region_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat region_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: ioports.region_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: region_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: region_count
- Source anchor: ioports
