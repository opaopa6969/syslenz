...
## NAME
`filesystems.filesystems` - metric signal from `filesystems` (filesystems)
...
This signal (filesystems.filesystems) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: filesystems.filesystems indicated a meaningful state transition.
