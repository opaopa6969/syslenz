...
## NAME
`dns.options` - metric signal from `dns` (options)
...
This signal (dns.options) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat options as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: dns.options indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: options was treated as evidence in a chain, not a singleton verdict.
