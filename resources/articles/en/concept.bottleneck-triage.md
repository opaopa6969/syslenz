...
## NAME
`concept.bottleneck-triage` - foundational systems concept for diagnosis
...
## Core Thesis
`Bottleneck Triage` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.bottleneck-triage) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat bottleneck-triage as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around bottleneck-triage.
...
### Episode 1
A single bottleneck narrative failed until packet path and queue behavior were modeled together.

### Episode 2
Scoped throttling plus queue observation outperformed blanket limits.
...
### Decision Record
- Primary claim: concept.bottleneck-triage indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: bottleneck-triage was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: bottleneck-triage
...
- Field in focus: bottleneck-triage
