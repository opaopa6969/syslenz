# Source Guide: file-nr

## NAME
`sourceguide.file-nr` - source-level operational reading guide
...
`file-nr` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.file-nr) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat file-nr as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for file-nr.
...
### Decision Record
- Primary claim: sourceguide.file-nr indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: file-nr was treated as evidence in a chain, not a singleton verdict.
