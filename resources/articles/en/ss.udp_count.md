...
## NAME
`ss.udp_count` - metric signal from `ss` (udp_count)
...
This signal (ss.udp_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat udp_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: ss.udp_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: udp_count was treated as evidence in a chain, not a singleton verdict.
