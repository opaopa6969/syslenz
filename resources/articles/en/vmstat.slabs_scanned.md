...
## NAME
`vmstat.slabs_scanned` - metric signal from `vmstat` (slabs_scanned)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `slabs_scanned` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `slabs_scanned` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.slabs_scanned`
- Source: `vmstat` (`slabs_scanned`)
- Field: `slabs_scanned`
...
This field is strongest for separating vmstat reclaim signal around `slabs_scanned` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
During a latency spike, reclaim-related counters moved before user-visible error rate changed.

For `slabs_scanned`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `slabs_scanned`.
...
This field is a manifestation of **virtual memory state transitions (`slabs_scanned`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`slabs_scanned`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `slabs_scanned`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `slabs_scanned`
- T-5m: first vmstat reclaim or scan anomaly candidate around `slabs_scanned`
- T0: user symptom confirmed with vmstat-side context around `slabs_scanned`
- T+3m: first hypothesis written with vmstat reclaim assumption for `slabs_scanned`
- T+6m: cross-source validation including scheduler or writeback for `slabs_scanned`
- T+10m: mitigation applied with rollback guard in the vmstat path for `slabs_scanned`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `slabs_scanned`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `slabs_scanned`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `slabs_scanned`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `slabs_scanned`
...
### Decision Record
- Primary claim: vmstat.slabs_scanned indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: slabs_scanned was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `slabs_scanned`.
...
- Field anchor: slabs_scanned
- Source anchor: vmstat (`slabs_scanned`)
...
### Debrief Questions
- Which vmstat-side step around `slabs_scanned` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `slabs_scanned` versus side effect?
- What vmstat instrumentation change around `slabs_scanned` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: slabs_scanned

## Failure Archetype Matrix
- Archetype A (`slabs_scanned`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`slabs_scanned`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`slabs_scanned`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: slabs_scanned
