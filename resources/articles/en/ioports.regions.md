...
## NAME
`ioports.regions` - metric signal from `ioports` (regions)
...
This signal (ioports.regions) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat regions as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: ioports.regions indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: regions was treated as evidence in a chain, not a singleton verdict.
