...
## NAME
`partitions.partitions` - metric signal from `partitions` (partitions)
...
3. Pair one sibling metric in `partitions` plus one pressure/queue metric from another source.
...
This signal (partitions.partitions) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: partitions.partitions indicated a meaningful state transition.
