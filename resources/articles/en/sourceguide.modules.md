# Source Guide: modules

## NAME
`sourceguide.modules` - source-level operational reading guide
...
`modules` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.modules) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat modules as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for modules.
...
### Decision Record
- Primary claim: sourceguide.modules indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: modules was treated as evidence in a chain, not a singleton verdict.
