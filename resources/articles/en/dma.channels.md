...
## NAME
`dma.channels` - metric signal from `dma` (channels)
...
This signal (dma.channels) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat channels as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: dma.channels indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: channels was treated as evidence in a chain, not a singleton verdict.
