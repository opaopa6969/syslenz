...
## NAME
`vmstat.htlb_buddy_alloc_fail` - metric signal from `vmstat` (htlb_buddy_alloc_fail)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `htlb_buddy_alloc_fail` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `htlb_buddy_alloc_fail` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.htlb_buddy_alloc_fail`
- Source: `vmstat` (`htlb_buddy_alloc_fail`)
...
This field is strongest for separating vmstat reclaim signal around `htlb_buddy_alloc_fail` from side-effect noise when scan and reclaim trends diverge.
...
For `htlb_buddy_alloc_fail`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `htlb_buddy_alloc_fail`.
...
This field is a manifestation of **virtual memory state transitions (`htlb_buddy_alloc_fail`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`htlb_buddy_alloc_fail`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `htlb_buddy_alloc_fail`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `htlb_buddy_alloc_fail`
- T-5m: first vmstat reclaim or scan anomaly candidate around `htlb_buddy_alloc_fail`
- T0: user symptom confirmed with vmstat-side context around `htlb_buddy_alloc_fail`
- T+3m: first hypothesis written with vmstat reclaim assumption for `htlb_buddy_alloc_fail`
- T+6m: cross-source validation including scheduler or writeback for `htlb_buddy_alloc_fail`
- T+10m: mitigation applied with rollback guard in the vmstat path for `htlb_buddy_alloc_fail`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `htlb_buddy_alloc_fail`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `htlb_buddy_alloc_fail`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `htlb_buddy_alloc_fail`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `htlb_buddy_alloc_fail`
...
### Decision Record
- Primary claim: vmstat.htlb_buddy_alloc_fail indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: htlb_buddy_alloc_fail was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `htlb_buddy_alloc_fail`.
...
- Field anchor: htlb_buddy_alloc_fail
- Source anchor: vmstat (`htlb_buddy_alloc_fail`)
...
### Debrief Questions
- Which vmstat-side step around `htlb_buddy_alloc_fail` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `htlb_buddy_alloc_fail` versus side effect?
- What vmstat instrumentation change around `htlb_buddy_alloc_fail` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: htlb_buddy_alloc_fail

## Failure Archetype Matrix
- Archetype A (`htlb_buddy_alloc_fail`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`htlb_buddy_alloc_fail`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`htlb_buddy_alloc_fail`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: htlb_buddy_alloc_fail
