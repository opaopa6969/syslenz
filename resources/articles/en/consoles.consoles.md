...
## NAME
`consoles.consoles` - metric signal from `consoles` (consoles)
...
This signal (consoles.consoles) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: consoles.consoles indicated a meaningful state transition.
