...
## NAME
`vmstat.pgalloc_movable` - metric signal from `vmstat` (pgalloc_movable)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `pgalloc_movable` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `pgalloc_movable` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.pgalloc_movable`
- Source: `vmstat` (`pgalloc_movable`)
...
This field is strongest for separating vmstat reclaim signal around `pgalloc_movable` from side-effect noise when scan and reclaim trends diverge.
...
For `pgalloc_movable`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `pgalloc_movable`.
...
This field is a manifestation of **virtual memory state transitions (`pgalloc_movable`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`pgalloc_movable`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `pgalloc_movable`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `pgalloc_movable`
- T-5m: first vmstat reclaim or scan anomaly candidate around `pgalloc_movable`
- T0: user symptom confirmed with vmstat-side context around `pgalloc_movable`
- T+3m: first hypothesis written with vmstat reclaim assumption for `pgalloc_movable`
- T+6m: cross-source validation including scheduler or writeback for `pgalloc_movable`
- T+10m: mitigation applied with rollback guard in the vmstat path for `pgalloc_movable`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `pgalloc_movable`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `pgalloc_movable`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `pgalloc_movable`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `pgalloc_movable`
...
### Decision Record
- Primary claim: vmstat.pgalloc_movable indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: pgalloc_movable was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `pgalloc_movable`.
...
- Field anchor: pgalloc_movable
- Source anchor: vmstat (`pgalloc_movable`)
...
### Debrief Questions
- Which vmstat-side step around `pgalloc_movable` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `pgalloc_movable` versus side effect?
- What vmstat instrumentation change around `pgalloc_movable` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: pgalloc_movable

## Failure Archetype Matrix
- Archetype A (`pgalloc_movable`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`pgalloc_movable`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`pgalloc_movable`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: pgalloc_movable
