...
## NAME
`sourceguide.dma` - source-level operational reading guide
...
`dma` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.dma) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat dma as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for dma.
...
### Decision Record
- Primary claim: sourceguide.dma indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: dma was treated as evidence in a chain, not a singleton verdict.
