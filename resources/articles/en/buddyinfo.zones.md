...
## NAME
`buddyinfo.zones` - metric signal from `buddyinfo` (zones)
...
This signal (buddyinfo.zones) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat zones as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: buddyinfo.zones indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: zones was treated as evidence in a chain, not a singleton verdict.
