...
## NAME
`sourceguide.version` - source-level operational reading guide
...
`version` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.version) is not only a number; it is an exposed edge of kernel state transitions.
...
### Focus
Source-level operational literacy for version.
...
### Decision Record
- Primary claim: sourceguide.version indicated a meaningful state transition.
