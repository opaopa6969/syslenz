...
## NAME
`mounts.count` - metric signal from `mounts` (count)
...
This signal (mounts.count) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: mounts.count indicated a meaningful state transition.
