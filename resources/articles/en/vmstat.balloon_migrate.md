...
## NAME
`vmstat.balloon_migrate` - metric signal from `vmstat` (balloon_migrate)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `balloon_migrate` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `balloon_migrate` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.balloon_migrate`
- Source: `vmstat` (`balloon_migrate`)
...
This field is strongest for separating vmstat reclaim signal around `balloon_migrate` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `balloon_migrate`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `balloon_migrate`.
...
This field is a manifestation of **virtual memory state transitions (`balloon_migrate`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`balloon_migrate`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `balloon_migrate`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `balloon_migrate`
- T-5m: first vmstat reclaim or scan anomaly candidate around `balloon_migrate`
- T0: user symptom confirmed with vmstat-side context around `balloon_migrate`
- T+3m: first hypothesis written with vmstat reclaim assumption for `balloon_migrate`
- T+6m: cross-source validation including scheduler or writeback for `balloon_migrate`
- T+10m: mitigation applied with rollback guard in the vmstat path for `balloon_migrate`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `balloon_migrate`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `balloon_migrate`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `balloon_migrate`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `balloon_migrate`
...
### Decision Record
- Primary claim: vmstat.balloon_migrate indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: balloon_migrate was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `balloon_migrate`.
...
- Field anchor: balloon_migrate
- Source anchor: vmstat (`balloon_migrate`)
...
### Debrief Questions
- Which vmstat-side step around `balloon_migrate` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `balloon_migrate` versus side effect?
- What vmstat instrumentation change around `balloon_migrate` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: balloon_migrate

## Failure Archetype Matrix
- Archetype A (`balloon_migrate`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`balloon_migrate`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`balloon_migrate`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: balloon_migrate
