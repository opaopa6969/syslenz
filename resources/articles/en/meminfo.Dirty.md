...
## NAME
`meminfo.Dirty` - metric signal from `meminfo` (Dirty)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `Dirty` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `Dirty` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `Dirty` from side-effect noise when free/reclaimable trends diverge.
...
For `Dirty`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Dirty`.
...
This field is a manifestation of **memory accounting surfaces (Dirty)**.

- Kernel path (Dirty): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (Dirty): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (Dirty): vmstat reclaim counters and pressure metrics.
...
### Decision Record
- Primary claim: meminfo.Dirty indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Dirty was treated as evidence in a chain, not a singleton verdict.
