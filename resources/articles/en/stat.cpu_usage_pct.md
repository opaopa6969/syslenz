...
## NAME
`stat.cpu_usage_pct` - metric signal from `stat` (cpu_usage_pct)
...
This signal (stat.cpu_usage_pct) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cpu_usage_pct as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: stat.cpu_usage_pct indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cpu_usage_pct was treated as evidence in a chain, not a singleton verdict.
