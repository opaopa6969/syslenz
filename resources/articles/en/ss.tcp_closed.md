...
## NAME
`ss.tcp_closed` - metric signal from `ss` (tcp_closed)
...
This signal (ss.tcp_closed) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat tcp_closed as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: ss.tcp_closed indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: tcp_closed was treated as evidence in a chain, not a singleton verdict.
