...
## NAME
`vmstat.drop_slab` - metric signal from `vmstat` (drop_slab)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `drop_slab` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `drop_slab` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating vmstat reclaim signal around `drop_slab` from side-effect noise when scan and reclaim trends diverge.
...
For `drop_slab`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `drop_slab`.
...
This field is a manifestation of **virtual memory state transitions (`drop_slab`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`drop_slab`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `drop_slab`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `drop_slab`
- T-5m: first vmstat reclaim or scan anomaly candidate around `drop_slab`
- T0: user symptom confirmed with vmstat-side context around `drop_slab`
- T+3m: first hypothesis written with vmstat reclaim assumption for `drop_slab`
- T+6m: cross-source validation including scheduler or writeback for `drop_slab`
- T+10m: mitigation applied with rollback guard in the vmstat path for `drop_slab`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `drop_slab`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `drop_slab`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `drop_slab`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `drop_slab`
...
### Decision Record
- Primary claim: vmstat.drop_slab indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: drop_slab was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `drop_slab`.
...
- Field anchor: drop_slab
- Source anchor: vmstat (`drop_slab`)
...
### Debrief Questions
- Which vmstat-side step around `drop_slab` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `drop_slab` versus side effect?
- What vmstat instrumentation change around `drop_slab` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: drop_slab

## Failure Archetype Matrix
- Archetype A (`drop_slab`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`drop_slab`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`drop_slab`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: drop_slab
