...
## NAME
`meminfo.SwapCached` - metric signal from `meminfo` (SwapCached)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `SwapCached` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `SwapCached` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `SwapCached` from side-effect noise when free/reclaimable trends diverge.

## Field Episode (MemInfo Lens)
Swap-related movement stayed low at first, then accelerated when cache headroom collapsed.

For `SwapCached`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `SwapCached`.
...
This field is a manifestation of **memory accounting surfaces (SwapCached)**.

- Kernel path (SwapCached): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (SwapCached): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (SwapCached): vmstat reclaim counters and pressure metrics.
...
### Decision Record
- Primary claim: meminfo.SwapCached indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: SwapCached was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: SwapCached
