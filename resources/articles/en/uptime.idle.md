...
## NAME
`uptime.idle` - metric signal from `uptime` (idle)
...
This signal (uptime.idle) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat idle as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: uptime.idle indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: idle was treated as evidence in a chain, not a singleton verdict.
