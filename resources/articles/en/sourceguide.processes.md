# Source Guide: processes

## NAME
`sourceguide.processes` - source-level operational reading guide
...
`processes` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.processes) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat processes as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for processes.
...
### Decision Record
- Primary claim: sourceguide.processes indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: processes was treated as evidence in a chain, not a singleton verdict.
