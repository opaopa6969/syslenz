...
## NAME
`concept.cross-metric-reading` - foundational systems concept for diagnosis
...
## Core Thesis
`Cross-Metric Reading` is less about one metric and more about choosing the right layer, timeline, and comparison set.
...
This signal (concept.cross-metric-reading) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cross-metric-reading as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around cross-metric-reading.
...
### Decision Record
- Primary claim: concept.cross-metric-reading indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cross-metric-reading was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: cross-metric-reading
...
- Field in focus: cross-metric-reading
