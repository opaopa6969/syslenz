...
## NAME
`sourceguide.ss` - source-level operational reading guide
...
`ss` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.ss) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat Netid State Recv-Q Send-Q Local Address:Port Peer Address:PortProcess as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for ss.
...
### Decision Record
- Primary claim: sourceguide.ss indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: ss was treated as evidence in a chain, not a singleton verdict.
