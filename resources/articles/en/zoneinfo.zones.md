...
## NAME
`zoneinfo.zones` - metric signal from `zoneinfo` (zones)
...
This signal (zoneinfo.zones) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: zoneinfo.zones indicated a meaningful state transition.
