...
## NAME
`stat.cpu_idle` - metric signal from `stat` (cpu_idle)
...
This signal (stat.cpu_idle) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat cpu_idle as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: stat.cpu_idle indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: cpu_idle was treated as evidence in a chain, not a singleton verdict.
