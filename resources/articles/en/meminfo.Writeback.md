...
## NAME
`meminfo.Writeback` - metric signal from `meminfo` (Writeback)

## WHY NOW
Read this when meminfo headroom and reclaimability behavior around `Writeback` is noisy and you need a fast, defensible decision.
If you cannot tell headroom erosion around `Writeback` from side effect, use this article to order evidence before tuning.
...
This field is strongest for separating meminfo headroom signal around `Writeback` from side-effect noise when free/reclaimable trends diverge.
...
For `Writeback`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Writeback`.
...
This field is a manifestation of **memory accounting surfaces (Writeback)**.

- Kernel path (Writeback): allocator, page cache, slab, reclaim boundaries.
- Typical trigger (Writeback): cache growth/shrink, allocation bursts, background reclaim.
- Cross-check (Writeback): vmstat reclaim counters and pressure metrics.
...
### Decision Record
- Primary claim: meminfo.Writeback indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Writeback was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (MemInfo)
Field under practice: Writeback
