...
## NAME
`conntrack.conntrack_max` - metric signal from `conntrack` (conntrack_max)
...
This signal (conntrack.conntrack_max) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat conntrack_max as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: conntrack.conntrack_max indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: conntrack_max was treated as evidence in a chain, not a singleton verdict.
