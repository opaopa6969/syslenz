# Source Guide: filesystems

## NAME
`sourceguide.filesystems` - source-level operational reading guide
...
`filesystems` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.filesystems) is not only a number; it is an exposed edge of kernel state transitions.
...
### Focus
Source-level operational literacy for filesystems.
...
### Decision Record
- Primary claim: sourceguide.filesystems indicated a meaningful state transition.
