# Source Guide: partitions

## NAME
`sourceguide.partitions` - source-level operational reading guide
...
`partitions` is a protocol-oriented telemetry surface. It helps connect socket behavior, packet lifecycle, and user-visible latency.
...
This signal (sourceguide.partitions) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat partitions as one scene in a longer diagnostic narrative.
...
### Focus
Source-level operational literacy for partitions.
...
### Decision Record
- Primary claim: sourceguide.partitions indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: partitions was treated as evidence in a chain, not a singleton verdict.
