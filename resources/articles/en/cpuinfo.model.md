...
## NAME
`cpuinfo.model` - metric signal from `cpuinfo` (model)
...
This signal (cpuinfo.model) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat model as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: cpuinfo.model indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: model was treated as evidence in a chain, not a singleton verdict.
