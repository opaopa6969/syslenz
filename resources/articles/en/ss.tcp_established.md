...
## NAME
`ss.tcp_established` - metric signal from `ss` (tcp_established)
...
## Metric Snapshot
- ID: `ss.tcp_established`
- Source: `ss`
- Field: `tcp_established`
- Domain: socket posture
...
## Episode
Socket state drift revealed handshake stalls before app error rate increased.
...
3. Pair one sibling metric in `ss` plus one pressure/queue metric from another source.
...
This signal (ss.tcp_established) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat tcp_established as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: ss.tcp_established indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: tcp_established was treated as evidence in a chain, not a singleton verdict.
...
- Field in focus: tcp_established
