...
## NAME
`locks.lock_count` - metric signal from `locks` (lock_count)
...
3. Pair one sibling metric in `locks` plus one pressure/queue metric from another source.
...
This signal (locks.lock_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat lock_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: locks.lock_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: lock_count was treated as evidence in a chain, not a singleton verdict.
