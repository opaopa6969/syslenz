# major_fault_distribution

## NAME
`vmstat.major_fault_distribution` - metric signal from `vmstat` (major_fault_distribution)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `major_fault_distribution` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `major_fault_distribution` from side effect, use this article to order evidence before tuning.
...
- Source: `vmstat` (`major_fault_distribution`)
- Field: `major_fault_distribution`
...
This field is strongest for separating vmstat reclaim signal around `major_fault_distribution` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `major_fault_distribution`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `major_fault_distribution`.
...
This field is a manifestation of **virtual memory state transitions (`major_fault_distribution`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`major_fault_distribution`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `major_fault_distribution`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `major_fault_distribution`
- T-5m: first vmstat reclaim or scan anomaly candidate around `major_fault_distribution`
- T0: user symptom confirmed with vmstat-side context around `major_fault_distribution`
- T+3m: first hypothesis written with vmstat reclaim assumption for `major_fault_distribution`
- T+6m: cross-source validation including scheduler or writeback for `major_fault_distribution`
- T+10m: mitigation applied with rollback guard in the vmstat path for `major_fault_distribution`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `major_fault_distribution`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `major_fault_distribution`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `major_fault_distribution`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `major_fault_distribution`
...
### Decision Record
- Primary claim: vmstat.major_fault_distribution indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: major_fault_distribution was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `major_fault_distribution`.
...
- Field anchor: major_fault_distribution
- Source anchor: vmstat (`major_fault_distribution`)
...
### Debrief Questions
- Which vmstat-side step around `major_fault_distribution` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `major_fault_distribution` versus side effect?
- What vmstat instrumentation change around `major_fault_distribution` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: major_fault_distribution

## Failure Archetype Matrix
- Archetype A (`major_fault_distribution`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`major_fault_distribution`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`major_fault_distribution`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: major_fault_distribution
