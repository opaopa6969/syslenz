...
## NAME
`iomem.region_count` - metric signal from `iomem` (region_count)
...
3. Pair one sibling metric in `iomem` plus one pressure/queue metric from another source.
...
This signal (iomem.region_count) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: iomem.region_count indicated a meaningful state transition.
