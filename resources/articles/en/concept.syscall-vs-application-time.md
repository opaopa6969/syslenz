# Application Time vs Syscall Time: Where Latency Lives

## NAME
`concept.syscall-vs-application-time` - foundational systems concept for diagnosis
...
Teams often optimize app logic while most wall time is spent waiting across kernel boundaries.

## Episode
An endpoint had expensive business logic on paper, but real latency came from blocking network/file waits and wakeup delays.

## Reading Pattern
1. map request path to major syscalls.
2. correlate with iowait, pressure, runqueue, and interrupt signals.
3. validate whether optimization changes total wall time or only CPU slices.

## Action Guide
- reduce unnecessary blocking operations.
- coalesce syscall frequency where possible.
- tune timeout/retry policy to avoid amplification.

## Systems Narrative (Systems)

This signal (concept.syscall-vs-application-time) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat syscall-vs-application-time as one scene in a longer diagnostic narrative.
...
### Focus
Conceptual model calibration around syscall-vs-application-time.
...
## Field Note: Syscall Time Attribution

Application teams often optimize code paths while waiting time dominates outside user space.
Split time into user compute, syscall active work, and blocked wait segments before deciding ownership.
Ownership decisions become fair only after that split, and remediation speed improves when teams share the same attribution model.

## Ownership Handshake

Before escalation, agree on a shared time-attribution table across app, kernel, and platform teams.
It removes blame loops and turns discussion into measurable action.
...
### Decision Record
- Primary claim: concept.syscall-vs-application-time indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: syscall-vs-application-time was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: syscall-vs-application-time
...
- Field in focus: syscall-vs-application-time
