...
## NAME
`net/netstat.TcpExt_TCPToZeroWindowAdv` - metric signal from `net/netstat` (TcpExt_TCPToZeroWindowAdv)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_TCPToZeroWindowAdv`
- Source: `net/netstat`
- Field: `TcpExt_TCPToZeroWindowAdv`
...
For `TcpExt_TCPToZeroWindowAdv`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_TCPToZeroWindowAdv`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_TCPToZeroWindowAdv`)**.

- Kernel path (Network `TcpExt_TCPToZeroWindowAdv`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_TCPToZeroWindowAdv`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_TCPToZeroWindowAdv`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_TCPToZeroWindowAdv indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_TCPToZeroWindowAdv was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: TcpExt_TCPToZeroWindowAdv
...
### Anchor (TCP Network)
Field under practice: TcpExt_TCPToZeroWindowAdv

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `TcpExt_TCPToZeroWindowAdv`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: TcpExt_TCPToZeroWindowAdv
