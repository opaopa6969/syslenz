...
## NAME
`net/snmp.Icmp_InCsumErrors` - metric signal from `net/snmp` (Icmp_InCsumErrors)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_InCsumErrors`
- Source: `net/snmp`
- Field: `Icmp_InCsumErrors`
...
For `Icmp_InCsumErrors`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_InCsumErrors`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_InCsumErrors indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_InCsumErrors was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_InCsumErrors
