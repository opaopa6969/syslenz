# TcpExt_TCPSackShifted

## NAME
`net/netstat.TcpExt_TCPSackShifted` - metric signal from `net/netstat` (TcpExt_TCPSackShifted)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_TCPSackShifted`
- Source: `net/netstat`
- Field: `TcpExt_TCPSackShifted`
...
For `TcpExt_TCPSackShifted`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_TCPSackShifted`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_TCPSackShifted`)**.

- Kernel path (Network `TcpExt_TCPSackShifted`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_TCPSackShifted`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_TCPSackShifted`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_TCPSackShifted indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_TCPSackShifted was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: TcpExt_TCPSackShifted
...
### Anchor (TCP Network)
Field under practice: TcpExt_TCPSackShifted

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `TcpExt_TCPSackShifted`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: TcpExt_TCPSackShifted
