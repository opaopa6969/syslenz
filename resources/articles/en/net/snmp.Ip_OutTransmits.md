...
## NAME
`net/snmp.Ip_OutTransmits` - metric signal from `net/snmp` (Ip_OutTransmits)
...
## Metric Snapshot
- ID: `net/snmp.Ip_OutTransmits`
- Source: `net/snmp`
- Field: `Ip_OutTransmits`
...
For `Ip_OutTransmits`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_OutTransmits`.
...
### Decision Record
- Primary claim: net/snmp.Ip_OutTransmits indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_OutTransmits was treated as evidence in a chain, not a singleton verdict.
