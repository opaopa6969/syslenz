...
## NAME
`net/udp.sockets` - metric signal from `net/udp` (sockets)
...
This signal (net/udp.sockets) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: net/udp.sockets indicated a meaningful state transition.
