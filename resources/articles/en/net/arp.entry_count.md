...
## NAME
`net/arp.entry_count` - metric signal from `net/arp` (entry_count)
...
3. Pair one sibling metric in `net/arp` plus one pressure/queue metric from another source.
...
This field is a manifestation of **extended networking edge conditions (`entry_count`)**.

- Kernel path (Network `entry_count`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `entry_count`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `entry_count`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/arp.entry_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat entry_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/arp.entry_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: entry_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: entry_count
- Source anchor: net/arp
