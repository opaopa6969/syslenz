...
## NAME
`net/snmp.Ip_Forwarding` - metric signal from `net/snmp` (Ip_Forwarding)
...
For `Ip_Forwarding`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_Forwarding`.
...
### Decision Record
- Primary claim: net/snmp.Ip_Forwarding indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_Forwarding was treated as evidence in a chain, not a singleton verdict.
