...
## NAME
`net/sockstat.UDPLITE_inuse` - metric signal from `net/sockstat` (UDPLITE_inuse)
...
3. Pair one sibling metric in `net/sockstat` plus one pressure/queue metric from another source.
...
This field is a manifestation of **extended networking edge conditions (`UDPLITE_inuse`)**.

- Kernel path (Network `UDPLITE_inuse`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `UDPLITE_inuse`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `UDPLITE_inuse`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.UDPLITE_inuse) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat UDPLITE_inuse as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.UDPLITE_inuse indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UDPLITE_inuse was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: UDPLITE_inuse
- Source anchor: net/sockstat
