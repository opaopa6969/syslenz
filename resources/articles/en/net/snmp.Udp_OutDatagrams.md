...
## NAME
`net/snmp.Udp_OutDatagrams` - metric signal from `net/snmp` (Udp_OutDatagrams)
...
## Metric Snapshot
- ID: `net/snmp.Udp_OutDatagrams`
- Source: `net/snmp`
- Field: `Udp_OutDatagrams`
...
For `Udp_OutDatagrams`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Udp_OutDatagrams`.
...
### Decision Record
- Primary claim: net/snmp.Udp_OutDatagrams indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Udp_OutDatagrams was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (UDP Network)
Field under practice: Udp_OutDatagrams
...
- Field in focus: Udp_OutDatagrams
