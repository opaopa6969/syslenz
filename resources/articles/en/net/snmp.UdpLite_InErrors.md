...
## NAME
`net/snmp.UdpLite_InErrors` - metric signal from `net/snmp` (UdpLite_InErrors)
...
## Metric Snapshot
- ID: `net/snmp.UdpLite_InErrors`
- Source: `net/snmp`
- Field: `UdpLite_InErrors`
...
For `UdpLite_InErrors`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `UdpLite_InErrors`.
...
### Decision Record
- Primary claim: net/snmp.UdpLite_InErrors indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UdpLite_InErrors was treated as evidence in a chain, not a singleton verdict.
