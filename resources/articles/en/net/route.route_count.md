...
## NAME
`net/route.route_count` - metric signal from `net/route` (route_count)
...
3. Pair one sibling metric in `net/route` plus one pressure/queue metric from another source.
...
This field is a manifestation of **extended networking edge conditions (`route_count`)**.

- Kernel path (Network `route_count`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `route_count`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `route_count`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/route.route_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat route_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/route.route_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: route_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: route_count
- Source anchor: net/route
