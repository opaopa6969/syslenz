...
## NAME
`net/dev.total_rx` - metric signal from `net/dev` (total_rx)
...
This field is a manifestation of **extended networking edge conditions (`total_rx`)**.

- Kernel path (Network `total_rx`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `total_rx`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `total_rx`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/dev.total_rx) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat total_rx as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/dev.total_rx indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: total_rx was treated as evidence in a chain, not a singleton verdict.
