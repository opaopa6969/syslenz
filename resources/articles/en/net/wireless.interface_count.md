...
## NAME
`net/wireless.interface_count` - metric signal from `net/wireless` (interface_count)
...
## Metric Snapshot
- ID: `net/wireless.interface_count`
- Source: `net/wireless`
...
3. Pair one sibling metric in `net/wireless` plus one pressure/queue metric from another source.
...
This field is a manifestation of **extended networking edge conditions (`interface_count`)**.

- Kernel path (Network `interface_count`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `interface_count`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `interface_count`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/wireless.interface_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat interface_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/wireless.interface_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: interface_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: interface_count
- Source anchor: net/wireless
...
- Field in focus: interface_count
