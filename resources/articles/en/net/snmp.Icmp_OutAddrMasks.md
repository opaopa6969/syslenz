...
## NAME
`net/snmp.Icmp_OutAddrMasks` - metric signal from `net/snmp` (Icmp_OutAddrMasks)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutAddrMasks`
- Source: `net/snmp`
- Field: `Icmp_OutAddrMasks`
...
For `Icmp_OutAddrMasks`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutAddrMasks`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutAddrMasks indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutAddrMasks was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutAddrMasks
