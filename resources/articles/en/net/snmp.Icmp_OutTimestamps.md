...
## NAME
`net/snmp.Icmp_OutTimestamps` - metric signal from `net/snmp` (Icmp_OutTimestamps)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutTimestamps`
- Source: `net/snmp`
- Field: `Icmp_OutTimestamps`
...
For `Icmp_OutTimestamps`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutTimestamps`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutTimestamps indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutTimestamps was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutTimestamps
