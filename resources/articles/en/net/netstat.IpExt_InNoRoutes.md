...
## NAME
`net/netstat.IpExt_InNoRoutes` - metric signal from `net/netstat` (IpExt_InNoRoutes)
...
## Metric Snapshot
- ID: `net/netstat.IpExt_InNoRoutes`
- Source: `net/netstat`
- Field: `IpExt_InNoRoutes`
...
For `IpExt_InNoRoutes`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `IpExt_InNoRoutes`.
...
This field is a manifestation of **extended networking edge conditions (`IpExt_InNoRoutes`)**.

- Kernel path (Network `IpExt_InNoRoutes`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `IpExt_InNoRoutes`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `IpExt_InNoRoutes`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.IpExt_InNoRoutes indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: IpExt_InNoRoutes was treated as evidence in a chain, not a singleton verdict.
