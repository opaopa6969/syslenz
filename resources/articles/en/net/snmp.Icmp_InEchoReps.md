...
## NAME
`net/snmp.Icmp_InEchoReps` - metric signal from `net/snmp` (Icmp_InEchoReps)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_InEchoReps`
- Source: `net/snmp`
- Field: `Icmp_InEchoReps`
...
For `Icmp_InEchoReps`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_InEchoReps`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_InEchoReps indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_InEchoReps was treated as evidence in a chain, not a singleton verdict.
