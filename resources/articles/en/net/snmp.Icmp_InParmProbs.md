...
## NAME
`net/snmp.Icmp_InParmProbs` - metric signal from `net/snmp` (Icmp_InParmProbs)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_InParmProbs`
- Source: `net/snmp`
- Field: `Icmp_InParmProbs`
...
For `Icmp_InParmProbs`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_InParmProbs`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_InParmProbs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_InParmProbs was treated as evidence in a chain, not a singleton verdict.
