...
## NAME
`net/snmp.Icmp_OutRedirects` - metric signal from `net/snmp` (Icmp_OutRedirects)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutRedirects`
- Source: `net/snmp`
- Field: `Icmp_OutRedirects`
...
For `Icmp_OutRedirects`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutRedirects`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutRedirects indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutRedirects was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutRedirects
