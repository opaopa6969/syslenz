...
## NAME
`net/netstat.IpExt_InMcastPkts` - metric signal from `net/netstat` (IpExt_InMcastPkts)
...
## Metric Snapshot
- ID: `net/netstat.IpExt_InMcastPkts`
- Source: `net/netstat`
- Field: `IpExt_InMcastPkts`
...
For `IpExt_InMcastPkts`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `IpExt_InMcastPkts`.
...
This field is a manifestation of **extended networking edge conditions (`IpExt_InMcastPkts`)**.

- Kernel path (Network `IpExt_InMcastPkts`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `IpExt_InMcastPkts`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `IpExt_InMcastPkts`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.IpExt_InMcastPkts indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: IpExt_InMcastPkts was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: IpExt_InMcastPkts
...
### Anchor (IP Network)
Field under practice: IpExt_InMcastPkts

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `IpExt_InMcastPkts`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: IpExt_InMcastPkts
