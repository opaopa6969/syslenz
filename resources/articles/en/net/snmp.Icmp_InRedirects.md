...
## NAME
`net/snmp.Icmp_InRedirects` - metric signal from `net/snmp` (Icmp_InRedirects)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_InRedirects`
- Source: `net/snmp`
- Field: `Icmp_InRedirects`
...
For `Icmp_InRedirects`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_InRedirects`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_InRedirects indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_InRedirects was treated as evidence in a chain, not a singleton verdict.
