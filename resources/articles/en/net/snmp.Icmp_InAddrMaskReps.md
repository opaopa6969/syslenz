...
## NAME
`net/snmp.Icmp_InAddrMaskReps` - metric signal from `net/snmp` (Icmp_InAddrMaskReps)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_InAddrMaskReps`
- Source: `net/snmp`
- Field: `Icmp_InAddrMaskReps`
...
For `Icmp_InAddrMaskReps`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_InAddrMaskReps`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_InAddrMaskReps indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_InAddrMaskReps was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_InAddrMaskReps
