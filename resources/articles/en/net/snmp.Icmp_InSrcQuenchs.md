...
## NAME
`net/snmp.Icmp_InSrcQuenchs` - metric signal from `net/snmp` (Icmp_InSrcQuenchs)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_InSrcQuenchs`
- Source: `net/snmp`
- Field: `Icmp_InSrcQuenchs`
...
For `Icmp_InSrcQuenchs`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_InSrcQuenchs`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_InSrcQuenchs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_InSrcQuenchs was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_InSrcQuenchs
