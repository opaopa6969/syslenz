...
## NAME
`net/sockstat.TCP_orphan` - metric signal from `net/sockstat` (TCP_orphan)
...
This field is a manifestation of **extended networking edge conditions (`TCP_orphan`)**.

- Kernel path (Network `TCP_orphan`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TCP_orphan`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TCP_orphan`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.TCP_orphan) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat TCP_orphan as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.TCP_orphan indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TCP_orphan was treated as evidence in a chain, not a singleton verdict.
