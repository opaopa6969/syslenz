# TcpExt_TCPSackMerged

## NAME
`net/netstat.TcpExt_TCPSackMerged` - metric signal from `net/netstat` (TcpExt_TCPSackMerged)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_TCPSackMerged`
- Source: `net/netstat`
- Field: `TcpExt_TCPSackMerged`
...
For `TcpExt_TCPSackMerged`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_TCPSackMerged`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_TCPSackMerged`)**.

- Kernel path (Network `TcpExt_TCPSackMerged`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_TCPSackMerged`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_TCPSackMerged`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_TCPSackMerged indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_TCPSackMerged was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: TcpExt_TCPSackMerged
...
### Anchor (TCP Network)
Field under practice: TcpExt_TCPSackMerged

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `TcpExt_TCPSackMerged`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: TcpExt_TCPSackMerged
