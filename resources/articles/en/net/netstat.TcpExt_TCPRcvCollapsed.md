...
## NAME
`net/netstat.TcpExt_TCPRcvCollapsed` - metric signal from `net/netstat` (TcpExt_TCPRcvCollapsed)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_TCPRcvCollapsed`
- Source: `net/netstat`
- Field: `TcpExt_TCPRcvCollapsed`
...
For `TcpExt_TCPRcvCollapsed`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_TCPRcvCollapsed`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_TCPRcvCollapsed`)**.

- Kernel path (Network `TcpExt_TCPRcvCollapsed`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_TCPRcvCollapsed`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_TCPRcvCollapsed`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_TCPRcvCollapsed indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_TCPRcvCollapsed was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: TcpExt_TCPRcvCollapsed
...
### Anchor (TCP Network)
Field under practice: TcpExt_TCPRcvCollapsed

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `TcpExt_TCPRcvCollapsed`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: TcpExt_TCPRcvCollapsed
