...
## NAME
`net/snmp.Tcp_PassiveOpens` - metric signal from `net/snmp` (Tcp_PassiveOpens)
...
## Metric Snapshot
- ID: `net/snmp.Tcp_PassiveOpens`
- Source: `net/snmp`
- Field: `Tcp_PassiveOpens`
...
For `Tcp_PassiveOpens`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Tcp_PassiveOpens`.
...
### Decision Record
- Primary claim: net/snmp.Tcp_PassiveOpens indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Tcp_PassiveOpens was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (TCP Network)
Field under practice: Tcp_PassiveOpens
...
- Field in focus: Tcp_PassiveOpens
