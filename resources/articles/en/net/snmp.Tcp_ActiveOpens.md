...
## NAME
`net/snmp.Tcp_ActiveOpens` - metric signal from `net/snmp` (Tcp_ActiveOpens)
...
## Metric Snapshot
- ID: `net/snmp.Tcp_ActiveOpens`
- Source: `net/snmp`
- Field: `Tcp_ActiveOpens`
...
For `Tcp_ActiveOpens`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Tcp_ActiveOpens`.
...
### Decision Record
- Primary claim: net/snmp.Tcp_ActiveOpens indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Tcp_ActiveOpens was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (TCP Network)
Field under practice: Tcp_ActiveOpens
...
- Field in focus: Tcp_ActiveOpens
