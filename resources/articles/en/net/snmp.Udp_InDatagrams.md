...
## NAME
`net/snmp.Udp_InDatagrams` - metric signal from `net/snmp` (Udp_InDatagrams)
...
## Metric Snapshot
- ID: `net/snmp.Udp_InDatagrams`
- Source: `net/snmp`
- Field: `Udp_InDatagrams`
...
For `Udp_InDatagrams`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Udp_InDatagrams`.
...
### Decision Record
- Primary claim: net/snmp.Udp_InDatagrams indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Udp_InDatagrams was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (UDP Network)
Field under practice: Udp_InDatagrams
...
- Field in focus: Udp_InDatagrams
