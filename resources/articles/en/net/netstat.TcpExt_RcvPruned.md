...
## NAME
`net/netstat.TcpExt_RcvPruned` - metric signal from `net/netstat` (TcpExt_RcvPruned)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_RcvPruned`
- Source: `net/netstat`
- Field: `TcpExt_RcvPruned`
...
For `TcpExt_RcvPruned`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_RcvPruned`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_RcvPruned`)**.

- Kernel path (Network `TcpExt_RcvPruned`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_RcvPruned`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_RcvPruned`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_RcvPruned indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_RcvPruned was treated as evidence in a chain, not a singleton verdict.
