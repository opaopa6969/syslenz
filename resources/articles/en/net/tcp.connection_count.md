...
## NAME
`net/tcp.connection_count` - metric signal from `net/tcp` (connection_count)
...
This field is a manifestation of **extended networking edge conditions (`connection_count`)**.

- Kernel path (Network `connection_count`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `connection_count`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `connection_count`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/tcp.connection_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat connection_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/tcp.connection_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: connection_count was treated as evidence in a chain, not a singleton verdict.
...
- Field in focus: connection_count
