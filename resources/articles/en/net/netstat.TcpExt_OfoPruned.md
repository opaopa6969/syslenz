...
## NAME
`net/netstat.TcpExt_OfoPruned` - metric signal from `net/netstat` (TcpExt_OfoPruned)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_OfoPruned`
- Source: `net/netstat`
- Field: `TcpExt_OfoPruned`
...
For `TcpExt_OfoPruned`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_OfoPruned`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_OfoPruned`)**.

- Kernel path (Network `TcpExt_OfoPruned`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_OfoPruned`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_OfoPruned`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_OfoPruned indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_OfoPruned was treated as evidence in a chain, not a singleton verdict.
