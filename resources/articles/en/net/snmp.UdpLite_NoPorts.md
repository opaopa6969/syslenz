...
## NAME
`net/snmp.UdpLite_NoPorts` - metric signal from `net/snmp` (UdpLite_NoPorts)
...
## Metric Snapshot
- ID: `net/snmp.UdpLite_NoPorts`
- Source: `net/snmp`
- Field: `UdpLite_NoPorts`
...
For `UdpLite_NoPorts`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `UdpLite_NoPorts`.
...
### Decision Record
- Primary claim: net/snmp.UdpLite_NoPorts indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UdpLite_NoPorts was treated as evidence in a chain, not a singleton verdict.
