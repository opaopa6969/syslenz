...
## NAME
`net/snmp.Icmp_OutTimestampReps` - metric signal from `net/snmp` (Icmp_OutTimestampReps)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutTimestampReps`
- Source: `net/snmp`
- Field: `Icmp_OutTimestampReps`
...
For `Icmp_OutTimestampReps`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutTimestampReps`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutTimestampReps indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutTimestampReps was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutTimestampReps
