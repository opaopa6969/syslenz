...
## NAME
`net/snmp.Ip_OutDiscards` - metric signal from `net/snmp` (Ip_OutDiscards)
...
## Metric Snapshot
- ID: `net/snmp.Ip_OutDiscards`
- Source: `net/snmp`
- Field: `Ip_OutDiscards`
...
For `Ip_OutDiscards`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_OutDiscards`.
...
### Decision Record
- Primary claim: net/snmp.Ip_OutDiscards indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_OutDiscards was treated as evidence in a chain, not a singleton verdict.
