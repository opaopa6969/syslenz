...
## NAME
`net/snmp.Icmp_InTimestamps` - metric signal from `net/snmp` (Icmp_InTimestamps)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_InTimestamps`
- Source: `net/snmp`
- Field: `Icmp_InTimestamps`
...
For `Icmp_InTimestamps`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_InTimestamps`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_InTimestamps indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_InTimestamps was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_InTimestamps
