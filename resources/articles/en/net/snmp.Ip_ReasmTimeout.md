...
## NAME
`net/snmp.Ip_ReasmTimeout` - metric signal from `net/snmp` (Ip_ReasmTimeout)
...
## Metric Snapshot
- ID: `net/snmp.Ip_ReasmTimeout`
- Source: `net/snmp`
- Field: `Ip_ReasmTimeout`
...
For `Ip_ReasmTimeout`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_ReasmTimeout`.
...
### Decision Record
- Primary claim: net/snmp.Ip_ReasmTimeout indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_ReasmTimeout was treated as evidence in a chain, not a singleton verdict.
