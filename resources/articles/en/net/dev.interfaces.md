...
## NAME
`net/dev.interfaces` - metric signal from `net/dev` (interfaces)
...
This signal (net/dev.interfaces) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: net/dev.interfaces indicated a meaningful state transition.
