...
## NAME
`net/sockstat.UDP_mem` - metric signal from `net/sockstat` (UDP_mem)
...
This field is a manifestation of **extended networking edge conditions (`UDP_mem`)**.

- Kernel path (Network `UDP_mem`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `UDP_mem`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `UDP_mem`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.UDP_mem) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat UDP_mem as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.UDP_mem indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UDP_mem was treated as evidence in a chain, not a singleton verdict.
