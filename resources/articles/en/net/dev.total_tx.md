...
## NAME
`net/dev.total_tx` - metric signal from `net/dev` (total_tx)
...
This field is a manifestation of **extended networking edge conditions (`total_tx`)**.

- Kernel path (Network `total_tx`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `total_tx`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `total_tx`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/dev.total_tx) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat total_tx as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/dev.total_tx indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: total_tx was treated as evidence in a chain, not a singleton verdict.
