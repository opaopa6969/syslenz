...
## NAME
`net/snmp.Icmp_OutDestUnreachs` - metric signal from `net/snmp` (Icmp_OutDestUnreachs)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutDestUnreachs`
- Source: `net/snmp`
- Field: `Icmp_OutDestUnreachs`
...
For `Icmp_OutDestUnreachs`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutDestUnreachs`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutDestUnreachs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutDestUnreachs was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutDestUnreachs
