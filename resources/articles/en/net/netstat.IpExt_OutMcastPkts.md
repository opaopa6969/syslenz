...
## NAME
`net/netstat.IpExt_OutMcastPkts` - metric signal from `net/netstat` (IpExt_OutMcastPkts)
...
## Metric Snapshot
- ID: `net/netstat.IpExt_OutMcastPkts`
- Source: `net/netstat`
- Field: `IpExt_OutMcastPkts`
...
For `IpExt_OutMcastPkts`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `IpExt_OutMcastPkts`.
...
This field is a manifestation of **extended networking edge conditions (`IpExt_OutMcastPkts`)**.

- Kernel path (Network `IpExt_OutMcastPkts`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `IpExt_OutMcastPkts`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `IpExt_OutMcastPkts`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.IpExt_OutMcastPkts indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: IpExt_OutMcastPkts was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: IpExt_OutMcastPkts
...
### Anchor (IP Network)
Field under practice: IpExt_OutMcastPkts

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `IpExt_OutMcastPkts`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: IpExt_OutMcastPkts
