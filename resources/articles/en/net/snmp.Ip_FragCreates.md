...
## NAME
`net/snmp.Ip_FragCreates` - metric signal from `net/snmp` (Ip_FragCreates)
...
## Metric Snapshot
- ID: `net/snmp.Ip_FragCreates`
- Source: `net/snmp`
- Field: `Ip_FragCreates`
...
For `Ip_FragCreates`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_FragCreates`.
...
### Decision Record
- Primary claim: net/snmp.Ip_FragCreates indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_FragCreates was treated as evidence in a chain, not a singleton verdict.
