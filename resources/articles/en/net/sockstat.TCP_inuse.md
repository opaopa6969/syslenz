...
## NAME
`net/sockstat.TCP_inuse` - metric signal from `net/sockstat` (TCP_inuse)
...
This field is a manifestation of **extended networking edge conditions (`TCP_inuse`)**.

- Kernel path (Network `TCP_inuse`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TCP_inuse`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TCP_inuse`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.TCP_inuse) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat TCP_inuse as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.TCP_inuse indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TCP_inuse was treated as evidence in a chain, not a singleton verdict.
