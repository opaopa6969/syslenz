...
## NAME
`net/route.routes` - metric signal from `net/route` (routes)
...
This field is a manifestation of **extended networking edge conditions (`routes`)**.

- Kernel path (Network `routes`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `routes`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `routes`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/route.routes) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat routes as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/route.routes indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: routes was treated as evidence in a chain, not a singleton verdict.
