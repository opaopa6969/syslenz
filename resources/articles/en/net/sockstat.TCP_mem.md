...
## NAME
`net/sockstat.TCP_mem` - metric signal from `net/sockstat` (TCP_mem)
...
This field is a manifestation of **extended networking edge conditions (`TCP_mem`)**.

- Kernel path (Network `TCP_mem`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TCP_mem`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TCP_mem`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.TCP_mem) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat TCP_mem as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.TCP_mem indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TCP_mem was treated as evidence in a chain, not a singleton verdict.
