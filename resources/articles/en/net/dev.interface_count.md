...
## NAME
`net/dev.interface_count` - metric signal from `net/dev` (interface_count)
...
3. Pair one sibling metric in `net/dev` plus one pressure/queue metric from another source.
...
This signal (net/dev.interface_count) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: net/dev.interface_count indicated a meaningful state transition.
