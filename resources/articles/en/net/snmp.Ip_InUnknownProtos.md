...
## NAME
`net/snmp.Ip_InUnknownProtos` - metric signal from `net/snmp` (Ip_InUnknownProtos)
...
## Metric Snapshot
- ID: `net/snmp.Ip_InUnknownProtos`
- Source: `net/snmp`
- Field: `Ip_InUnknownProtos`
...
For `Ip_InUnknownProtos`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_InUnknownProtos`.
...
### Decision Record
- Primary claim: net/snmp.Ip_InUnknownProtos indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_InUnknownProtos was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Ip_InUnknownProtos
