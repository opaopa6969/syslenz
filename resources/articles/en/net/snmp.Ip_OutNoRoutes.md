...
## NAME
`net/snmp.Ip_OutNoRoutes` - metric signal from `net/snmp` (Ip_OutNoRoutes)
...
## Metric Snapshot
- ID: `net/snmp.Ip_OutNoRoutes`
- Source: `net/snmp`
- Field: `Ip_OutNoRoutes`
...
For `Ip_OutNoRoutes`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_OutNoRoutes`.
...
### Decision Record
- Primary claim: net/snmp.Ip_OutNoRoutes indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_OutNoRoutes was treated as evidence in a chain, not a singleton verdict.
