...
## NAME
`net/udp.socket_count` - metric signal from `net/udp` (socket_count)
...
3. Pair one sibling metric in `net/udp` plus one pressure/queue metric from another source.
...
This signal (net/udp.socket_count) is not only a number; it is an exposed edge of kernel state transitions.
...
### Decision Record
- Primary claim: net/udp.socket_count indicated a meaningful state transition.
