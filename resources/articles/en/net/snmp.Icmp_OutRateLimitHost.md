...
## NAME
`net/snmp.Icmp_OutRateLimitHost` - metric signal from `net/snmp` (Icmp_OutRateLimitHost)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutRateLimitHost`
- Source: `net/snmp`
- Field: `Icmp_OutRateLimitHost`
...
For `Icmp_OutRateLimitHost`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutRateLimitHost`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutRateLimitHost indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutRateLimitHost was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutRateLimitHost
