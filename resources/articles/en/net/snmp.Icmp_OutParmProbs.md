...
## NAME
`net/snmp.Icmp_OutParmProbs` - metric signal from `net/snmp` (Icmp_OutParmProbs)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutParmProbs`
- Source: `net/snmp`
- Field: `Icmp_OutParmProbs`
...
For `Icmp_OutParmProbs`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutParmProbs`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutParmProbs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutParmProbs was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutParmProbs
