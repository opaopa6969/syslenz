...
## NAME
`net/snmp.Ip_ReasmReqds` - metric signal from `net/snmp` (Ip_ReasmReqds)
...
For `Ip_ReasmReqds`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_ReasmReqds`.
...
### Decision Record
- Primary claim: net/snmp.Ip_ReasmReqds indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_ReasmReqds was treated as evidence in a chain, not a singleton verdict.
