...
## NAME
`net/snmp.Icmp_OutErrors` - metric signal from `net/snmp` (Icmp_OutErrors)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutErrors`
- Source: `net/snmp`
- Field: `Icmp_OutErrors`
...
For `Icmp_OutErrors`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutErrors`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutErrors indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutErrors was treated as evidence in a chain, not a singleton verdict.
