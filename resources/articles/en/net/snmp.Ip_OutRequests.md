...
## NAME
`net/snmp.Ip_OutRequests` - metric signal from `net/snmp` (Ip_OutRequests)
...
## Metric Snapshot
- ID: `net/snmp.Ip_OutRequests`
- Source: `net/snmp`
- Field: `Ip_OutRequests`
...
For `Ip_OutRequests`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_OutRequests`.
...
### Decision Record
- Primary claim: net/snmp.Ip_OutRequests indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_OutRequests was treated as evidence in a chain, not a singleton verdict.
