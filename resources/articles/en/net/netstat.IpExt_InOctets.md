...
## NAME
`net/netstat.IpExt_InOctets` - metric signal from `net/netstat` (IpExt_InOctets)
...
## Metric Snapshot
- ID: `net/netstat.IpExt_InOctets`
- Source: `net/netstat`
- Field: `IpExt_InOctets`
...
For `IpExt_InOctets`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `IpExt_InOctets`.
...
This field is a manifestation of **extended networking edge conditions (`IpExt_InOctets`)**.

- Kernel path (Network `IpExt_InOctets`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `IpExt_InOctets`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `IpExt_InOctets`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.IpExt_InOctets indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: IpExt_InOctets was treated as evidence in a chain, not a singleton verdict.
