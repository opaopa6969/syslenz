...
## NAME
`net/sockstat.RAW_inuse` - metric signal from `net/sockstat` (RAW_inuse)
...
This field is a manifestation of **extended networking edge conditions (`RAW_inuse`)**.

- Kernel path (Network `RAW_inuse`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `RAW_inuse`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `RAW_inuse`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.RAW_inuse) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat RAW_inuse as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.RAW_inuse indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: RAW_inuse was treated as evidence in a chain, not a singleton verdict.
