...
## NAME
`net/snmp.Icmp_OutSrcQuenchs` - metric signal from `net/snmp` (Icmp_OutSrcQuenchs)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutSrcQuenchs`
- Source: `net/snmp`
- Field: `Icmp_OutSrcQuenchs`
...
For `Icmp_OutSrcQuenchs`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutSrcQuenchs`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutSrcQuenchs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutSrcQuenchs was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutSrcQuenchs
