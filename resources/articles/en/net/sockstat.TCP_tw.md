...
## NAME
`net/sockstat.TCP_tw` - metric signal from `net/sockstat` (TCP_tw)
...
This field is a manifestation of **extended networking edge conditions (`TCP_tw`)**.

- Kernel path (Network `TCP_tw`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TCP_tw`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TCP_tw`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.TCP_tw) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat TCP_tw as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.TCP_tw indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TCP_tw was treated as evidence in a chain, not a singleton verdict.
