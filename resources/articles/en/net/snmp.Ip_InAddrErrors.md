...
## NAME
`net/snmp.Ip_InAddrErrors` - metric signal from `net/snmp` (Ip_InAddrErrors)
...
## Metric Snapshot
- ID: `net/snmp.Ip_InAddrErrors`
- Source: `net/snmp`
- Field: `Ip_InAddrErrors`
...
For `Ip_InAddrErrors`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_InAddrErrors`.
...
### Decision Record
- Primary claim: net/snmp.Ip_InAddrErrors indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_InAddrErrors was treated as evidence in a chain, not a singleton verdict.
