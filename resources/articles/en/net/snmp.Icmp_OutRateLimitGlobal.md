...
## NAME
`net/snmp.Icmp_OutRateLimitGlobal` - metric signal from `net/snmp` (Icmp_OutRateLimitGlobal)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutRateLimitGlobal`
- Source: `net/snmp`
- Field: `Icmp_OutRateLimitGlobal`
...
For `Icmp_OutRateLimitGlobal`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutRateLimitGlobal`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutRateLimitGlobal indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutRateLimitGlobal was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutRateLimitGlobal
...
### Anchor (ICMP Network)
Field under practice: Icmp_OutRateLimitGlobal

## Failure Archetype Matrix
- Archetype A: localized IP-path instability hidden inside globally healthy aggregates.
- Archetype B (Network `Icmp_OutRateLimitGlobal`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: control-plane side effects mistaken for network capacity failure.
- Field in focus: Icmp_OutRateLimitGlobal
