...
## NAME
`net/netstat.TcpExt_TCPDSACKOldSent` - metric signal from `net/netstat` (TcpExt_TCPDSACKOldSent)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_TCPDSACKOldSent`
- Source: `net/netstat`
- Field: `TcpExt_TCPDSACKOldSent`
...
For `TcpExt_TCPDSACKOldSent`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_TCPDSACKOldSent`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_TCPDSACKOldSent`)**.

- Kernel path (Network `TcpExt_TCPDSACKOldSent`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_TCPDSACKOldSent`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_TCPDSACKOldSent`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_TCPDSACKOldSent indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_TCPDSACKOldSent was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: TcpExt_TCPDSACKOldSent
...
### Anchor (TCP Network)
Field under practice: TcpExt_TCPDSACKOldSent

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `TcpExt_TCPDSACKOldSent`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: TcpExt_TCPDSACKOldSent
