...
## NAME
`net/netstat.TcpExt_TCPHPHits` - metric signal from `net/netstat` (TcpExt_TCPHPHits)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_TCPHPHits`
- Source: `net/netstat`
- Field: `TcpExt_TCPHPHits`
...
For `TcpExt_TCPHPHits`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_TCPHPHits`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_TCPHPHits`)**.

- Kernel path (Network `TcpExt_TCPHPHits`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_TCPHPHits`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_TCPHPHits`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_TCPHPHits indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_TCPHPHits was treated as evidence in a chain, not a singleton verdict.
