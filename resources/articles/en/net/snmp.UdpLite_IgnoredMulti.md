...
## NAME
`net/snmp.UdpLite_IgnoredMulti` - metric signal from `net/snmp` (UdpLite_IgnoredMulti)
...
## Metric Snapshot
- ID: `net/snmp.UdpLite_IgnoredMulti`
- Source: `net/snmp`
- Field: `UdpLite_IgnoredMulti`
...
For `UdpLite_IgnoredMulti`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `UdpLite_IgnoredMulti`.
...
### Decision Record
- Primary claim: net/snmp.UdpLite_IgnoredMulti indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UdpLite_IgnoredMulti was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: UdpLite_IgnoredMulti
...
Field under practice: UdpLite_IgnoredMulti

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `UdpLite_IgnoredMulti`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: UdpLite_IgnoredMulti
