...
## NAME
`net/snmp.Tcp_RtoMin` - metric signal from `net/snmp` (Tcp_RtoMin)
...
For `Tcp_RtoMin`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Tcp_RtoMin`.
...
### Decision Record
- Primary claim: net/snmp.Tcp_RtoMin indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Tcp_RtoMin was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (TCP Network)
Field under practice: Tcp_RtoMin
