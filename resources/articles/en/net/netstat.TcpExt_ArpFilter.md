...
## NAME
`net/netstat.TcpExt_ArpFilter` - metric signal from `net/netstat` (TcpExt_ArpFilter)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_ArpFilter`
- Source: `net/netstat`
- Field: `TcpExt_ArpFilter`
...
For `TcpExt_ArpFilter`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_ArpFilter`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_ArpFilter`)**.

- Kernel path (Network `TcpExt_ArpFilter`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_ArpFilter`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_ArpFilter`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_ArpFilter indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_ArpFilter was treated as evidence in a chain, not a singleton verdict.
