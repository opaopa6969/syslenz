...
## NAME
`net/netstat.TcpExt_TW` - metric signal from `net/netstat` (TcpExt_TW)
...
For `TcpExt_TW`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_TW`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_TW`)**.

- Kernel path (Network `TcpExt_TW`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_TW`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_TW`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_TW indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_TW was treated as evidence in a chain, not a singleton verdict.
