...
## NAME
`net/snmp.Ip_InReceives` - metric signal from `net/snmp` (Ip_InReceives)
...
For `Ip_InReceives`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_InReceives`.
...
### Decision Record
- Primary claim: net/snmp.Ip_InReceives indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_InReceives was treated as evidence in a chain, not a singleton verdict.
