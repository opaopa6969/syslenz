...
## NAME
`net/snmp.Ip_ForwDatagrams` - metric signal from `net/snmp` (Ip_ForwDatagrams)
...
## Metric Snapshot
- ID: `net/snmp.Ip_ForwDatagrams`
- Source: `net/snmp`
- Field: `Ip_ForwDatagrams`
...
For `Ip_ForwDatagrams`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_ForwDatagrams`.
...
### Decision Record
- Primary claim: net/snmp.Ip_ForwDatagrams indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_ForwDatagrams was treated as evidence in a chain, not a singleton verdict.
