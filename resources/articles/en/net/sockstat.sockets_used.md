...
## NAME
`net/sockstat.sockets_used` - metric signal from `net/sockstat` (sockets_used)
...
This field is a manifestation of **extended networking edge conditions (`sockets_used`)**.

- Kernel path (Network `sockets_used`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `sockets_used`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `sockets_used`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.sockets_used) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat sockets_used as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.sockets_used indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: sockets_used was treated as evidence in a chain, not a singleton verdict.
