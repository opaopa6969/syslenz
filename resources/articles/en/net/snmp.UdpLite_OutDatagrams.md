...
## NAME
`net/snmp.UdpLite_OutDatagrams` - metric signal from `net/snmp` (UdpLite_OutDatagrams)
...
## Metric Snapshot
- ID: `net/snmp.UdpLite_OutDatagrams`
- Source: `net/snmp`
- Field: `UdpLite_OutDatagrams`
...
For `UdpLite_OutDatagrams`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `UdpLite_OutDatagrams`.
...
### Decision Record
- Primary claim: net/snmp.UdpLite_OutDatagrams indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UdpLite_OutDatagrams was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: UdpLite_OutDatagrams
...
Field under practice: UdpLite_OutDatagrams

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `UdpLite_OutDatagrams`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: UdpLite_OutDatagrams
