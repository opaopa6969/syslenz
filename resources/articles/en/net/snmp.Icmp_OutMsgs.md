...
## NAME
`net/snmp.Icmp_OutMsgs` - metric signal from `net/snmp` (Icmp_OutMsgs)
...
For `Icmp_OutMsgs`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutMsgs`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutMsgs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutMsgs was treated as evidence in a chain, not a singleton verdict.
