...
## NAME
`net/arp.entries` - metric signal from `net/arp` (entries)
...
This field is a manifestation of **extended networking edge conditions (`entries`)**.

- Kernel path (Network `entries`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `entries`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `entries`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/arp.entries) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat entries as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/arp.entries indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: entries was treated as evidence in a chain, not a singleton verdict.
