...
## NAME
`net/snmp.Tcp_InErrs` - metric signal from `net/snmp` (Tcp_InErrs)
...
For `Tcp_InErrs`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Tcp_InErrs`.
...
### Decision Record
- Primary claim: net/snmp.Tcp_InErrs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Tcp_InErrs was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (TCP Network)
Field under practice: Tcp_InErrs
