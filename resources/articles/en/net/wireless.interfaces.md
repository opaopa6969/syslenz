...
## NAME
`net/wireless.interfaces` - metric signal from `net/wireless` (interfaces)
...
This field is a manifestation of **extended networking edge conditions (`interfaces`)**.

- Kernel path (Network `interfaces`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `interfaces`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `interfaces`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/wireless.interfaces) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat interfaces as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/wireless.interfaces indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: interfaces was treated as evidence in a chain, not a singleton verdict.
