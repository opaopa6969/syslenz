...
## NAME
`net/snmp.Tcp_InCsumErrors` - metric signal from `net/snmp` (Tcp_InCsumErrors)
...
## Metric Snapshot
- ID: `net/snmp.Tcp_InCsumErrors`
- Source: `net/snmp`
- Field: `Tcp_InCsumErrors`
...
For `Tcp_InCsumErrors`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Tcp_InCsumErrors`.
...
### Decision Record
- Primary claim: net/snmp.Tcp_InCsumErrors indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Tcp_InCsumErrors was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (TCP Network)
Field under practice: Tcp_InCsumErrors
...
- Field in focus: Tcp_InCsumErrors
