...
## NAME
`net/snmp.Icmp_OutEchoReps` - metric signal from `net/snmp` (Icmp_OutEchoReps)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutEchoReps`
- Source: `net/snmp`
- Field: `Icmp_OutEchoReps`
...
For `Icmp_OutEchoReps`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutEchoReps`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutEchoReps indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutEchoReps was treated as evidence in a chain, not a singleton verdict.
