...
## NAME
`net/snmp.UdpLite_InDatagrams` - metric signal from `net/snmp` (UdpLite_InDatagrams)
...
## Metric Snapshot
- ID: `net/snmp.UdpLite_InDatagrams`
- Source: `net/snmp`
- Field: `UdpLite_InDatagrams`
...
For `UdpLite_InDatagrams`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `UdpLite_InDatagrams`.
...
### Decision Record
- Primary claim: net/snmp.UdpLite_InDatagrams indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UdpLite_InDatagrams was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: UdpLite_InDatagrams
...
Field under practice: UdpLite_InDatagrams

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `UdpLite_InDatagrams`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: UdpLite_InDatagrams
