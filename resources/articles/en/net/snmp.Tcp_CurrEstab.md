...
## NAME
`net/snmp.Tcp_CurrEstab` - metric signal from `net/snmp` (Tcp_CurrEstab)
...
For `Tcp_CurrEstab`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Tcp_CurrEstab`.
...
### Decision Record
- Primary claim: net/snmp.Tcp_CurrEstab indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Tcp_CurrEstab was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (TCP Network)
Field under practice: Tcp_CurrEstab
