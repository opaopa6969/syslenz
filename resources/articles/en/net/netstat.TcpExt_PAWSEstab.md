...
## NAME
`net/netstat.TcpExt_PAWSEstab` - metric signal from `net/netstat` (TcpExt_PAWSEstab)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_PAWSEstab`
- Source: `net/netstat`
- Field: `TcpExt_PAWSEstab`
...
For `TcpExt_PAWSEstab`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_PAWSEstab`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_PAWSEstab`)**.

- Kernel path (Network `TcpExt_PAWSEstab`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_PAWSEstab`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_PAWSEstab`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_PAWSEstab indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_PAWSEstab was treated as evidence in a chain, not a singleton verdict.
