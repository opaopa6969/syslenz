...
## NAME
`net/snmp.UdpLite_SndbufErrors` - metric signal from `net/snmp` (UdpLite_SndbufErrors)
...
## Metric Snapshot
- ID: `net/snmp.UdpLite_SndbufErrors`
- Source: `net/snmp`
- Field: `UdpLite_SndbufErrors`
...
For `UdpLite_SndbufErrors`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `UdpLite_SndbufErrors`.
...
### Decision Record
- Primary claim: net/snmp.UdpLite_SndbufErrors indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UdpLite_SndbufErrors was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: UdpLite_SndbufErrors
...
Field under practice: UdpLite_SndbufErrors

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `UdpLite_SndbufErrors`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: UdpLite_SndbufErrors
