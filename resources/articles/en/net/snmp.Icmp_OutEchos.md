...
## NAME
`net/snmp.Icmp_OutEchos` - metric signal from `net/snmp` (Icmp_OutEchos)
...
For `Icmp_OutEchos`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutEchos`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutEchos indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutEchos was treated as evidence in a chain, not a singleton verdict.
