...
## NAME
`net/snmp.Udp_RcvbufErrors` - metric signal from `net/snmp` (Udp_RcvbufErrors)
...
For `Udp_RcvbufErrors`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Udp_RcvbufErrors`.
...
### Decision Record
- Primary claim: net/snmp.Udp_RcvbufErrors indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Udp_RcvbufErrors was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (UDP Network)
Field under practice: Udp_RcvbufErrors
...
- Field in focus: Udp_RcvbufErrors
