...
## NAME
`net/snmp.UdpLite_RcvbufErrors` - metric signal from `net/snmp` (UdpLite_RcvbufErrors)
...
## Metric Snapshot
- ID: `net/snmp.UdpLite_RcvbufErrors`
- Source: `net/snmp`
- Field: `UdpLite_RcvbufErrors`
...
For `UdpLite_RcvbufErrors`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `UdpLite_RcvbufErrors`.
...
### Decision Record
- Primary claim: net/snmp.UdpLite_RcvbufErrors indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UdpLite_RcvbufErrors was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: UdpLite_RcvbufErrors
...
Field under practice: UdpLite_RcvbufErrors

## Failure Archetype Matrix
- Archetype A: localized transport instability hidden inside global aggregates.
- Archetype B (Network `UdpLite_RcvbufErrors`): anomaly amplification driven by wake-delay and queue coupling.
- Archetype C: policy side effects mistaken for network capacity failure.
- Field in focus: UdpLite_RcvbufErrors
