...
## NAME
`net/snmp.Udp_InCsumErrors` - metric signal from `net/snmp` (Udp_InCsumErrors)
...
## Metric Snapshot
- ID: `net/snmp.Udp_InCsumErrors`
- Source: `net/snmp`
- Field: `Udp_InCsumErrors`
...
For `Udp_InCsumErrors`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Udp_InCsumErrors`.
...
### Decision Record
- Primary claim: net/snmp.Udp_InCsumErrors indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Udp_InCsumErrors was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (UDP Network)
Field under practice: Udp_InCsumErrors
...
- Field in focus: Udp_InCsumErrors
