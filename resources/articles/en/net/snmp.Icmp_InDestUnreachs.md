...
## NAME
`net/snmp.Icmp_InDestUnreachs` - metric signal from `net/snmp` (Icmp_InDestUnreachs)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_InDestUnreachs`
- Source: `net/snmp`
- Field: `Icmp_InDestUnreachs`
...
For `Icmp_InDestUnreachs`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_InDestUnreachs`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_InDestUnreachs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_InDestUnreachs was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_InDestUnreachs
