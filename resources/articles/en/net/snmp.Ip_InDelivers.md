...
## NAME
`net/snmp.Ip_InDelivers` - metric signal from `net/snmp` (Ip_InDelivers)
...
For `Ip_InDelivers`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_InDelivers`.
...
### Decision Record
- Primary claim: net/snmp.Ip_InDelivers indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_InDelivers was treated as evidence in a chain, not a singleton verdict.
