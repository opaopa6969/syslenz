...
## NAME
`net/snmp.Ip_FragOKs` - metric signal from `net/snmp` (Ip_FragOKs)
...
For `Ip_FragOKs`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Ip_FragOKs`.
...
### Decision Record
- Primary claim: net/snmp.Ip_FragOKs indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Ip_FragOKs was treated as evidence in a chain, not a singleton verdict.
