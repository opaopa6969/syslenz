...
## NAME
`net/sockstat.TCP_alloc` - metric signal from `net/sockstat` (TCP_alloc)
...
This field is a manifestation of **extended networking edge conditions (`TCP_alloc`)**.

- Kernel path (Network `TCP_alloc`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TCP_alloc`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TCP_alloc`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.TCP_alloc) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat TCP_alloc as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.TCP_alloc indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TCP_alloc was treated as evidence in a chain, not a singleton verdict.
