...
## NAME
`net/unix.socket_count` - metric signal from `net/unix` (socket_count)
...
3. Pair one sibling metric in `net/unix` plus one pressure/queue metric from another source.
...
This field is a manifestation of **extended networking edge conditions (`socket_count`)**.

- Kernel path (Network `socket_count`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `socket_count`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `socket_count`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/unix.socket_count) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat socket_count as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/unix.socket_count indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: socket_count was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: socket_count
- Source anchor: net/unix
