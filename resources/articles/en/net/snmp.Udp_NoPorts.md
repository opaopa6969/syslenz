...
## NAME
`net/snmp.Udp_NoPorts` - metric signal from `net/snmp` (Udp_NoPorts)
...
For `Udp_NoPorts`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Udp_NoPorts`.
...
### Decision Record
- Primary claim: net/snmp.Udp_NoPorts indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Udp_NoPorts was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (UDP Network)
Field under practice: Udp_NoPorts
