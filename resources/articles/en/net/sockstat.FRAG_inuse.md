...
## NAME
`net/sockstat.FRAG_inuse` - metric signal from `net/sockstat` (FRAG_inuse)
...
This field is a manifestation of **extended networking edge conditions (`FRAG_inuse`)**.

- Kernel path (Network `FRAG_inuse`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `FRAG_inuse`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `FRAG_inuse`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.FRAG_inuse) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat FRAG_inuse as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.FRAG_inuse indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: FRAG_inuse was treated as evidence in a chain, not a singleton verdict.
