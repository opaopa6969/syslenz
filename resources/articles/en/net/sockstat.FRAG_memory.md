...
## NAME
`net/sockstat.FRAG_memory` - metric signal from `net/sockstat` (FRAG_memory)
...
This field is a manifestation of **extended networking edge conditions (`FRAG_memory`)**.

- Kernel path (Network `FRAG_memory`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `FRAG_memory`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `FRAG_memory`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.FRAG_memory) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat FRAG_memory as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.FRAG_memory indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: FRAG_memory was treated as evidence in a chain, not a singleton verdict.
