...
## NAME
`net/netstat.IpExt_InECT1Pkts` - metric signal from `net/netstat` (IpExt_InECT1Pkts)
...
## Metric Snapshot
- ID: `net/netstat.IpExt_InECT1Pkts`
- Source: `net/netstat`
- Field: `IpExt_InECT1Pkts`
...
For `IpExt_InECT1Pkts`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `IpExt_InECT1Pkts`.
...
This field is a manifestation of **extended networking edge conditions (`IpExt_InECT1Pkts`)**.

- Kernel path (Network `IpExt_InECT1Pkts`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `IpExt_InECT1Pkts`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `IpExt_InECT1Pkts`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.IpExt_InECT1Pkts indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: IpExt_InECT1Pkts was treated as evidence in a chain, not a singleton verdict.
