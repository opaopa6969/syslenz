...
## NAME
`net/sockstat.UDP_inuse` - metric signal from `net/sockstat` (UDP_inuse)
...
This field is a manifestation of **extended networking edge conditions (`UDP_inuse`)**.

- Kernel path (Network `UDP_inuse`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `UDP_inuse`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `UDP_inuse`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/sockstat.UDP_inuse) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat UDP_inuse as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/sockstat.UDP_inuse indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: UDP_inuse was treated as evidence in a chain, not a singleton verdict.
