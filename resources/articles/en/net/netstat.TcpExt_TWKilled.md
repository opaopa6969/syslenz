...
## NAME
`net/netstat.TcpExt_TWKilled` - metric signal from `net/netstat` (TcpExt_TWKilled)
...
## Metric Snapshot
- ID: `net/netstat.TcpExt_TWKilled`
- Source: `net/netstat`
- Field: `TcpExt_TWKilled`
...
For `TcpExt_TWKilled`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `TcpExt_TWKilled`.
...
This field is a manifestation of **extended networking edge conditions (`TcpExt_TWKilled`)**.

- Kernel path (Network `TcpExt_TWKilled`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `TcpExt_TWKilled`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `TcpExt_TWKilled`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.TcpExt_TWKilled indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: TcpExt_TWKilled was treated as evidence in a chain, not a singleton verdict.
