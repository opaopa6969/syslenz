...
## NAME
`net/snmp.Tcp_AttemptFails` - metric signal from `net/snmp` (Tcp_AttemptFails)
...
## Metric Snapshot
- ID: `net/snmp.Tcp_AttemptFails`
- Source: `net/snmp`
- Field: `Tcp_AttemptFails`
...
For `Tcp_AttemptFails`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Tcp_AttemptFails`.
...
### Decision Record
- Primary claim: net/snmp.Tcp_AttemptFails indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Tcp_AttemptFails was treated as evidence in a chain, not a singleton verdict.
...
### Anchor (TCP Network)
Field under practice: Tcp_AttemptFails
...
- Field in focus: Tcp_AttemptFails
