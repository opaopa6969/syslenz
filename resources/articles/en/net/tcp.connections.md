...
## NAME
`net/tcp.connections` - metric signal from `net/tcp` (connections)
...
This field is a manifestation of **extended networking edge conditions (`connections`)**.

- Kernel path (Network `connections`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `connections`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `connections`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/tcp.connections) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat connections as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/tcp.connections indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: connections was treated as evidence in a chain, not a singleton verdict.
