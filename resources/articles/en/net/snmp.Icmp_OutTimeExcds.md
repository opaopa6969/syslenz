...
## NAME
`net/snmp.Icmp_OutTimeExcds` - metric signal from `net/snmp` (Icmp_OutTimeExcds)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_OutTimeExcds`
- Source: `net/snmp`
- Field: `Icmp_OutTimeExcds`
...
For `Icmp_OutTimeExcds`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_OutTimeExcds`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_OutTimeExcds indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_OutTimeExcds was treated as evidence in a chain, not a singleton verdict.
...
- Field anchor: Icmp_OutTimeExcds
