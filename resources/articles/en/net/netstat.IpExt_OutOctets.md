...
## NAME
`net/netstat.IpExt_OutOctets` - metric signal from `net/netstat` (IpExt_OutOctets)
...
## Metric Snapshot
- ID: `net/netstat.IpExt_OutOctets`
- Source: `net/netstat`
- Field: `IpExt_OutOctets`
...
For `IpExt_OutOctets`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `IpExt_OutOctets`.
...
This field is a manifestation of **extended networking edge conditions (`IpExt_OutOctets`)**.

- Kernel path (Network `IpExt_OutOctets`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `IpExt_OutOctets`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `IpExt_OutOctets`): protocol counters plus application timeout/error rates.
...
### Decision Record
- Primary claim: net/netstat.IpExt_OutOctets indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: IpExt_OutOctets was treated as evidence in a chain, not a singleton verdict.
