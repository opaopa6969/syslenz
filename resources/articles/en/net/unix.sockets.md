...
## NAME
`net/unix.sockets` - metric signal from `net/unix` (sockets)
...
This field is a manifestation of **extended networking edge conditions (`sockets`)**.

- Kernel path (Network `sockets`): drop/retry/error pathways not obvious in throughput charts.
- Typical trigger (Network `sockets`): hidden retry loops and pathological connection patterns.
- Cross-check (Network `sockets`): protocol counters plus application timeout/error rates.

## Systems Narrative (Network)

This signal (net/unix.sockets) is not only a number; it is an exposed edge of kernel state transitions.
...
Treat sockets as one scene in a longer diagnostic narrative.
...
### Decision Record
- Primary claim: net/unix.sockets indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: sockets was treated as evidence in a chain, not a singleton verdict.
