...
## NAME
`net/snmp.Icmp_InAddrMasks` - metric signal from `net/snmp` (Icmp_InAddrMasks)
...
## Metric Snapshot
- ID: `net/snmp.Icmp_InAddrMasks`
- Source: `net/snmp`
- Field: `Icmp_InAddrMasks`
...
For `Icmp_InAddrMasks`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `Icmp_InAddrMasks`.
...
### Decision Record
- Primary claim: net/snmp.Icmp_InAddrMasks indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: Icmp_InAddrMasks was treated as evidence in a chain, not a singleton verdict.
