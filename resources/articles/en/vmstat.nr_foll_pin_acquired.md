...
## NAME
`vmstat.nr_foll_pin_acquired` - metric signal from `vmstat` (nr_foll_pin_acquired)

## WHY NOW
Read this when vmstat reclaim and scan behavior around `nr_foll_pin_acquired` is noisy and you need a fast, defensible decision.
If you cannot tell reclaim pressure around `nr_foll_pin_acquired` from side effect, use this article to order evidence before tuning.
...
## Metric Snapshot
- ID: `vmstat.nr_foll_pin_acquired`
- Source: `vmstat` (`nr_foll_pin_acquired`)
...
This field is strongest for separating vmstat reclaim signal around `nr_foll_pin_acquired` from side-effect noise when scan and reclaim trends diverge.

## Field Episode (VMStat Lens)
The field looked quiet in isolation, but trend context changed the operational decision.

For `nr_foll_pin_acquired`, the practical value comes from ordering evidence in time: what moved first, what followed, and what changed after mitigation.
...
## Action Loop
1. State a falsifiable hypothesis for `nr_foll_pin_acquired`.
...
This field is a manifestation of **virtual memory state transitions (`nr_foll_pin_acquired`)**.

- Kernel path: reclaim, compaction, page fault handling, swap policy.
- Typical trigger (`nr_foll_pin_acquired`): memory pressure or workload phase shift.
...
- This section mirrors man-page flow for vmstat reclaim analysis of `nr_foll_pin_acquired`: definition -> scan/reclaim context -> failure branches -> evidence order.
...
- T-10m: baseline snapshot with vmstat reclaim/scan phase annotation for `nr_foll_pin_acquired`
- T-5m: first vmstat reclaim or scan anomaly candidate around `nr_foll_pin_acquired`
- T0: user symptom confirmed with vmstat-side context around `nr_foll_pin_acquired`
- T+3m: first hypothesis written with vmstat reclaim assumption for `nr_foll_pin_acquired`
- T+6m: cross-source validation including scheduler or writeback for `nr_foll_pin_acquired`
- T+10m: mitigation applied with rollback guard in the vmstat path for `nr_foll_pin_acquired`
- T+15m: trend reaction checked for vmstat reclaim stabilization around `nr_foll_pin_acquired`
- T+30m: recovery confidence decision with vmstat and pressure confirmation for `nr_foll_pin_acquired`

### Evidence Quality Rubric (VMStat)
- Strong: ordered, cross-layer, and vmstat reclaim-consistent trend evidence for `nr_foll_pin_acquired`
- Medium: correlated movement without clear vmstat reclaim/scan boundary
- Weak: isolated value with no vmstat reclaim/scan context around `nr_foll_pin_acquired`
...
### Decision Record
- Primary claim: vmstat.nr_foll_pin_acquired indicated a meaningful state transition.
- Disproof attempt: identify one alternate cause and log why it failed.
- Action note: nr_foll_pin_acquired was treated as evidence in a chain, not a singleton verdict.

## Man-Page Crosswalk
- Process lens: Process: check runnable vs blocked expansion around vmstat inflection windows.
- Syscall lens: Syscall: identify page-fault and memory-touching call bursts aligned with the vmstat transitions for `nr_foll_pin_acquired`.
...
- Field anchor: nr_foll_pin_acquired
- Source anchor: vmstat (`nr_foll_pin_acquired`)
...
### Debrief Questions
- Which vmstat-side step around `nr_foll_pin_acquired` produced the highest confidence gain?
- Which step failed to reduce uncertainty about reclaim pressure around `nr_foll_pin_acquired` versus side effect?
- What vmstat instrumentation change around `nr_foll_pin_acquired` would accelerate this drill next time?

### Anchor (VMStat)
Field under practice: nr_foll_pin_acquired

## Failure Archetype Matrix
- Archetype A (`nr_foll_pin_acquired`): silent vmstat reclaim pressure where utilization looks safe but stalls rise.
- Archetype B (`nr_foll_pin_acquired`): vmstat reclaim oscillation causing repeated short recoveries and relapses.
- Archetype C (`nr_foll_pin_acquired`): vmstat scan/reclaim phase shift with delayed user-impact visibility.
- Field in focus: nr_foll_pin_acquired
