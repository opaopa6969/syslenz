...
## NAME
`meminfo.SwapFree` - `meminfo` の指標シグナル（SwapFree）

## なぜ今読むか
meminfo の余力/回収可能性挙動が `SwapFree` 周辺でノイジーで、短時間に防御可能な判断が必要な時に読みます。
`SwapFree` 周辺で余力劣化と副作用の切り分けが曖昧なら、調整前に証拠順序をここで固定します。
...
## 運用上の意味（MemInfo視点）
この field は、meminfo の余力主信号と副作用ノイズを、free/reclaimableトレンド分岐時に分離するのに最も有効です（`SwapFree`）。

## フィールドエピソード（MemInfo視点）
swap 系の変化は最初は小さく、cache 余力崩壊後に加速しました。

`SwapFree` の実務価値は、証拠の時間順を整理することで出ます。何が先に動き、何が後追いし、対策後に何が変わったかを追ってください。
...
この field は **メモリ会計面の発露（`SwapFree`）** です。

- カーネル経路（`SwapFree`）: allocator、page cache、slab、reclaim 境界。
- 主な契機（`SwapFree`）: cache 増減、割当バースト、バックグラウンド reclaim。
- 併読（`SwapFree`）: vmstat reclaim カウンタと pressure 指標。
...
### 判断記録
- 主張: meminfo.SwapFree は意味のある状態遷移を示した。
- 反証試行: 代替原因を1つ立て、なぜ棄却したかを記録する。
- 行動メモ: SwapFree は単独判定ではなく、証拠連鎖の一部として扱った。
