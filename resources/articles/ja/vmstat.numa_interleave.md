...
## NAME
`vmstat.numa_interleave` - `vmstat` の指標シグナル（numa_interleave）

## なぜ今読むか
vmstat `numa_interleave` の reclaim/scan 挙動がノイジーで、短時間に防御可能な判断が必要な時に読みます。
`numa_interleave` 周辺で reclaim圧力と副作用の切り分けが曖昧なら、調整前に証拠順序をここで固定します。
...
## 運用上の意味（VMStat視点）
この field は、vmstat `numa_interleave` の reclaim 主信号と副作用ノイズを、scan/reclaimトレンド分岐時に分離するのに最も有効です。

## フィールドエピソード（VMStat視点）
単体では静かに見える field でも、時系列文脈で運用判断が変わりました。

`numa_interleave` の実務価値は、証拠の時間順を整理することで出ます。何が先に動き、何が後追いし、対策後に何が変わったかを追ってください。
...
## アクションループ
1. `numa_interleave` に対する反証可能な仮説を置く。
2. 可逆な緩和策を適用する。
...
- 迷ったら本番で `numa_interleave` に関わる vmstat 関連メモリノブ変更前に SEE ALSO を辿って証拠を増やしてください。
...
- 緩和が10分遅れた場合、`numa_interleave` 連動のどの vmstat ユーザー指標が先に閾値を超えるか？
...
### ドリフト防止チェック（VMStat）
- 環境、負荷位相、`numa_interleave` を含む vmstat reclaimモードごとに基線メモを1つ維持する。
...
### 判断記録
- 主張: vmstat.numa_interleave は意味のある状態遷移を示した。
- 反証試行: 代替原因を1つ立て、なぜ棄却したかを記録する。
- 行動メモ: numa_interleave は単独判定ではなく、証拠連鎖の一部として扱った。
...
- 次回このドリルを速くする `numa_interleave` 周辺の vmstat 計測改善は何か？

### アンカー（VMStat）
練習対象field: numa_interleave
