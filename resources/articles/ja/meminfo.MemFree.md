...
## NAME
`meminfo.MemFree` - `meminfo` の指標シグナル（MemFree）

## なぜ今読むか
meminfo の余力/回収可能性挙動が `MemFree` 周辺でノイジーで、短時間に防御可能な判断が必要な時に読みます。
`MemFree` 周辺で余力劣化と副作用の切り分けが曖昧なら、調整前に証拠順序をここで固定します。
...
## 運用上の意味（MemInfo視点）
この field は、meminfo の余力主信号と副作用ノイズを、free/reclaimableトレンド分岐時に分離するのに最も有効です（`MemFree`）。

## フィールドエピソード（MemInfo視点）
単一チャートでは余力があるように見えても、関連 field では不安定域へ向けた浸食が進んでいました。

`MemFree` の実務価値は、証拠の時間順を整理することで出ます。何が先に動き、何が後追いし、対策後に何が変わったかを追ってください。
...
この field は **メモリ会計面の発露（`MemFree`）** です。

- カーネル経路（`MemFree`）: allocator、page cache、slab、reclaim 境界。
- 主な契機（`MemFree`）: cache 増減、割当バースト、バックグラウンド reclaim。
- 併読（`MemFree`）: vmstat reclaim カウンタと pressure 指標。

## ケースブック（MemInfo系）

### インシデント断面1（MemInfo）
ケースB（`MemFree`）: クロスソース照合で初期診断が反転した。

### インシデント断面2（MemInfo）
ケースC（`MemFree`）: 侵襲的変更より可逆対策で学習速度が上がった。

### インシデント断面3（MemInfo）
ケースA（`MemFree`）: 絶対値ではなくトレンドが最初の異常を示した。
...
### 判断記録
- 主張: meminfo.MemFree は意味のある状態遷移を示した。
- 反証試行: 代替原因を1つ立て、なぜ棄却したかを記録する。
- 行動メモ: MemFree は単独判定ではなく、証拠連鎖の一部として扱った。
