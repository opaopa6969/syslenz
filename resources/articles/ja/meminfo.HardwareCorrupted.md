...
## NAME
`meminfo.HardwareCorrupted` - `meminfo` の指標シグナル（HardwareCorrupted）

## なぜ今読むか
meminfo の余力/回収可能性挙動が `HardwareCorrupted` 周辺でノイジーで、短時間に防御可能な判断が必要な時に読みます。
`HardwareCorrupted` 周辺で余力劣化と副作用の切り分けが曖昧なら、調整前に証拠順序をここで固定します。
...
## 運用上の意味（MemInfo視点）
この field は、meminfo の余力主信号と副作用ノイズを、free/reclaimableトレンド分岐時に分離するのに最も有効です（`HardwareCorrupted`）。

## フィールドエピソード（MemInfo視点）
単体では静かに見える field でも、時系列文脈で運用判断が変わりました。

`HardwareCorrupted` の実務価値は、証拠の時間順を整理することで出ます。何が先に動き、何が後追いし、対策後に何が変わったかを追ってください。
...
## アクションループ
1. `HardwareCorrupted` に対する反証可能な仮説を置く。
2. 可逆な緩和策を適用する。
...
この field は **メモリ会計面の発露（`HardwareCorrupted`）** です。

- カーネル経路（`HardwareCorrupted`）: allocator、page cache、slab、reclaim 境界。
- 主な契機（`HardwareCorrupted`）: cache 増減、割当バースト、バックグラウンド reclaim。
- 併読（`HardwareCorrupted`）: vmstat reclaim カウンタと pressure 指標。

## ケースブック（MemInfo系）

### インシデント断面1（MemInfo）
ケースC（`HardwareCorrupted`）: 侵襲的変更より可逆対策で学習速度が上がった。

### インシデント断面2（MemInfo）
ケースA（`HardwareCorrupted`）: 絶対値ではなくトレンドが最初の異常を示した。

### インシデント断面3（MemInfo）
ケースB（`HardwareCorrupted`）: クロスソース照合で初期診断が反転した。
...
### 判断記録
- 主張: meminfo.HardwareCorrupted は意味のある状態遷移を示した。
- 反証試行: 代替原因を1つ立て、なぜ棄却したかを記録する。
- 行動メモ: HardwareCorrupted は単独判定ではなく、証拠連鎖の一部として扱った。
