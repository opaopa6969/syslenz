...
## NAME
`vmstat.pgfault` - `vmstat` の指標シグナル（pgfault）

## なぜ今読むか
vmstat `pgfault` の reclaim/scan 挙動がノイジーで、短時間に防御可能な判断が必要な時に読みます。
`pgfault` 周辺で reclaim圧力と副作用の切り分けが曖昧なら、調整前に証拠順序をここで固定します。
...
## 運用上の意味（VMStat視点）
この field は、vmstat `pgfault` の reclaim 主信号と副作用ノイズを、scan/reclaimトレンド分岐時に分離するのに最も有効です。

## フィールドエピソード（VMStat視点）
遅延悪化の局面で、ユーザー可視エラーより先に reclaim 系カウンタが動きました。

`pgfault` の実務価値は、証拠の時間順を整理することで出ます。何が先に動き、何が後追いし、対策後に何が変わったかを追ってください。
...
- 迷ったら本番で `pgfault` に関わる vmstat 関連メモリノブ変更前に SEE ALSO を辿って証拠を増やしてください。
...
- 緩和が10分遅れた場合、`pgfault` 連動のどの vmstat ユーザー指標が先に閾値を超えるか？
...
### ドリフト防止チェック（VMStat）
- 環境、負荷位相、`pgfault` を含む vmstat reclaimモードごとに基線メモを1つ維持する。
...
### 判断記録
- 主張: vmstat.pgfault は意味のある状態遷移を示した。
- 反証試行: 代替原因を1つ立て、なぜ棄却したかを記録する。
- 行動メモ: pgfault は単独判定ではなく、証拠連鎖の一部として扱った。
...
- 次回このドリルを速くする `pgfault` 周辺の vmstat 計測改善は何か？
