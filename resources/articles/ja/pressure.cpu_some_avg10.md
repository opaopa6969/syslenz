...
## NAME
`pressure.cpu_some_avg10` - `pressure` の指標シグナル（cpu_some_avg10）

## なぜ今読むか
pressure の stalled-time 挙動が `cpu_some_avg10` 周辺でノイジーで、短時間に防御可能な判断が必要な時に読みます。
`cpu_some_avg10` 周辺で stalled-time 主信号と副作用の切り分けが曖昧なら、調整前に証拠順序をここで固定します。
...
## 運用上の意味（Pressure視点）
この field は、pressure の stalled-time 主信号と副作用ノイズを、some/fullトレンド分岐時に分離するのに最も有効です（`cpu_some_avg10`）。

## フィールドエピソード（Pressure視点）
平均 pressure は許容でも、累積 stall 時間が tail latency 劣化を説明しました。

`cpu_some_avg10` の実務価値は、証拠の時間順を整理することで出ます。何が先に動き、何が後追いし、対策後に何が変わったかを追ってください。
...
## アクションループ
1. `cpu_some_avg10` に対する反証可能な仮説を置く。
2. 可逆な緩和策を適用する。
...
この field は **カーネル内 stall 時間蓄積の発露（`cpu_some_avg10`）** です。

- カーネル経路（`cpu_some_avg10`）: runnable 待ち、reclaim stall、I/O待ち連鎖。
- 主な契機（`cpu_some_avg10`）: CPU%に出にくい資源競合。
- 併読（`cpu_some_avg10`）: runqueue、reclaim、queue-depth。
...
### 判断記録
- 主張: pressure.cpu_some_avg10 は意味のある状態遷移を示した。
- 反証試行: 代替原因を1つ立て、なぜ棄却したかを記録する。
- 行動メモ: cpu_some_avg10 は単独判定ではなく、証拠連鎖の一部として扱った。
