...
`--web [addr:port]`（デフォルト `127.0.0.1:3000`）で起動（`web` feature が必要）。`src/web.rs` に実装。
...
`--serve [bind_addr]`（デフォルト `127.0.0.1:9100`）が `src/serve.rs` で軽量 TCP サーバーを起動。
...
**セキュリティ**: 認証なしで、デフォルトでは `127.0.0.1:9100`（ループバックのみ）にバインドします。外部公開する場合は、外部から接続できるバインドアドレスを明示し、ファイアウォールでアクセスを制限してください。
...
ローカライズされたフィールド説明の明示的な上書きは `src/i18n.rs` に
`(source_name, field_name)` をキーとして格納。上書きがないフィールドでは、
各パーサーが提供する説明を使用する。
...
`L` キーで実行時に `en` と `ja` のロケールを切替。v1.7.0 時点で、英語と
日本語の上書きテーブルは同じ 584 フィールドキーをそれぞれ持つ
（584/584 の上書きが対応）。日本語モードで上書きがないフィールドは、
パーサーの英語説明へフォールバックし、TUI では翻訳準備中として表示される。
