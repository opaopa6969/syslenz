...
デフォルトでは `127.0.0.1` にバインドします。ポートだけを渡した場合もループバックを使います。ネットワーク公開が必要な場合だけ `0.0.0.0:8080` のようにアドレスを明示してください。サーバーは待受アドレスを表示します:
...
- **RT** バッジ（または `r` キー）でリアルタイム更新の一時停止／再開。
- **AXIS** バッジ（または `s` キー）でダッシュボードの線グラフの軸を自動レンジとゼロ基点で切り替え。
...
- Web UIには認証がありません。`--web 127.0.0.1:3000` でループバックにバインドするか、ネットワーク公開時は認証付きリバースプロキシを使ってください。
